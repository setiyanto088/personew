	...
	";	
		...
		INSERT INTO `M_SUM_TV_DASH_PROG_RES_FULL`
						...
						A.ID_PROFILE,1 AS STS,0 as tpe FROM (
							...
							WHERE PERIODE = '".$periode."'
							AND PROFILE_ID = '".$id."'
							GROUP BY CHANNEL_NAME_PROG,PROGRAM
		...
		INSERT INTO `M_SUM_TV_DASH_PROG_DAYE_RES_FULL`
						...
						A.IDPRO,1 AS STS,0 as tpe FROM (
								...
								SELECT RESPID,CHANNEL_NAME_PROG CHANNEL,PROGRAM,WEIGHT,'".$periode."' PERIODE,formatDateTime(BEGIN_PROGRAM,'%Y-%m-%d') HARIS, 0 STS  
								FROM `CDR_EPG_RES_".$name_tbs_new."_SPLIT_STEP4_V2_2022` A JOIN
								CHANNEL_PARAM_FINAL B ON A.CHANNEL = B.CHANNEL_NAME
								WHERE RESPID IN (SELECT `CARDNO` AS people FROM PROFILE_CARDNO_RES WHERE M_TYPE = 0 AND ID_PROFILE = ".$id." )
							...
							WHERE PERIODE = '".$periode."'
							AND PROFILE_ID = '".$id."'
							GROUP BY CHANNEL_NAME_PROG,PROGRAM,formatDateTime(SPLIT_MINUTES,'%Y-%m-%d')
		...
		INSERT INTO `M_SUM_TV_DASH_PROG_DAYE_RES_FULL`
						...
						A.IDPRO,1 AS STS,0 as tpe FROM (
								...
								SELECT RESPID,CHANNEL_NAME_PROG CHANNEL,PROGRAM,BEGIN_PROGRAM,WEIGHT,'".$periode."' PERIODE,formatDateTime(BEGIN_PROGRAM,'%Y-%m-%d') HARIS, 0 STS  
								FROM `CDR_EPG_RES_".$name_tbs_new."_SPLIT_STEP4_V2_2022` A JOIN
								CHANNEL_PARAM_FINAL B ON A.CHANNEL = B.CHANNEL_NAME
								WHERE RESPID IN (SELECT `CARDNO` AS people FROM PROFILE_CARDNO_RES WHERE M_TYPE = 0 AND ID_PROFILE = ".$id." )
							...
							WHERE PERIODE = '".$periode."'
							AND PROFILE_ID = '".$id."'
							GROUP BY CHANNEL_NAME_PROG,PROGRAM,BEGIN_PROGRAM,formatDateTime(SPLIT_MINUTES,'%Y-%m-%d')
						) B ON A.CHANNEL = B.CHANNEL AND A.PROGRAM = B.PROGRAM AND A.HARIS = B.HARIS
						JOIN (
							SELECT CHANNEL,PROGRAM, `VIEWERS` AS UNIVERSE_IDX, `VIEWERS2` AS UNIVERSE_IDX_ALL, `DATE` FROM  `M_SUM_TV_DASH_PROG_DAY_RES` A
								WHERE A.TANGGAL = '".$periode."' AND A.ID_PROFILE = '1'  AND A.DATA_TYPE = 'TVR' AND A.`STATUS` = 1 
		...
		GROUP BY CHANNEL, DATE, START_TIME, NAMA_BRAND,ID_PROFILE,NET_PRICE
			...
			SELECT SUM(WEIGHT) AS VVV,ID_PROFILE FROM (
				SELECT RESPID,WEIGHT,ID_PROFILE FROM (
				 ...
				 where toDateTime(B.SPLIT_MINUTES) BETWEEN BEGIN_PROGRAM AND END_PROGRAM 
...
");
	...
	$commands = 'php /var/www/jobs/profile_jobs_res_p22_temp_cl_ext.php '.$id.' > /var/www/jobs/profile_jobs_res_p22_temp_cl_ext_'.$id.'res_2021.log 2>&1 & '; 
