    ...
    */
    ...
    protected function _check_login($username = '', $password = false)
    {
        ...
        if (!$this->config->item('auth_source') && $rest_auth == 'digest') { // for digest we do not have a password passed as argument
            return hash("sha512",$username.':'.$this->config->item('rest_realm').':'.(isset($valid_logins[$username])?$valid_logins[$username]:''));
        }
    ...
    protected function _prepare_digest_auth()
    {
        ...
        $A2 = hash("sha512",strtoupper($this->request->method).':'.$digest['uri']);
        $valid_response = hash("sha512",$A1.':'.$digest['nonce'].':'.$digest['nc'].':'.$digest['cnonce'].':'.$digest['qop'].':'.$A2);
    ...
    protected function _force_login($nonce = '')
    {
        if (strtolower( $this->config->item('rest_auth') ) == 'basic') {
            ...
            header('WWW-Authenticate: Digest realm="'.$this->config->item('rest_realm').'", qop="auth", nonce="'.$nonce.'", opaque="'.hash("sha512",$this->config->item('rest_realm')).'"');
        }
