    ...
    public function _calculateFormulaValue($formula, $cellID = null, PHPExcel_Cell $pCell = null)
    {
        ...
        if (($wsTitle{0} !== "\x00") && ($this->cyclicReferenceStack->onStack($wsCellReference))) {
            if ($this->cyclicFormulaCount <= 0) {
				...
				$raiseFormulaError = 'Cyclic Reference in Formula';
				return $raiseFormulaError;
    ...
    private function convertMatrixReferences($formula)
    {
        ...
        if (strpos($formula, '{') !== false) {
            ...
            if ($openCount < $closeCount) {
                if ($openCount > 0) {
					$raiseFormulaError = "Formula Error: Mismatched matrix braces '}'";
					return $raiseFormulaError;
                } else {
					$raiseFormulaError = "Formula Error: Unexpected '}' encountered";
				return $raiseFormulaError;
                }
...
//echo 'Element is a Positive number, not Plus operator', PHP_EOL;
                ++$index;                                                    //    Drop the redundant plus symbol
					...
					$raiseFormulaError = "Formula Error: Illegal character '~'";
					return $raiseFormulaError;                //        on the stack but not in the input expression
...
//echo 'Element is a Closing bracket', PHP_EOL;
                ...
                while (($o2 = $stack->pop()) && $o2['value'] != '(') {        //    Pop off the stack back to the last (
                    if ($o2 === null) {
						$raiseFormulaError = 'Formula Error: Unexpected closing brace ")"';
						return $raiseFormulaError; 
                    } else {
                        $output[] = $o2;
                    }
...
//echo 'PHPExcel function '.$functionName, PHP_EOL;
						...
						$raiseFormulaError = "Formula Error: Internal error, non-function on stack";
						return $raiseFormulaError; 
                    }
...
//echo PHP_EOL;
                    ...
                    if ($argumentCountError) {
                       $raiseFormulaError = "Formula Error: Wrong number of arguments for $functionName() function: $argumentCount given, ".$expectedArgumentCountString." expected";
						return $raiseFormulaError; 
                    }
                }
                ++$index;
...
//echo 'Element is a Function argument separator', PHP_EOL;
                while (($o2 = $stack->pop()) && $o2['value'] != '(') {        //    Pop off the stack back to the last (
                    if ($o2 === null) {
						$raiseFormulaError = "Formula Error: Unexpected ,";
						return $raiseFormulaError; 
                    } else {
...
//                    echo 'Element '.$val.' is a Cell reference<br />';
                    ...
                    if ($testPrevOp['value'] == ':') {
                        ...
                        if ($matches[2] == '') {
							...
							$raiseFormulaError = "3D Range references are not yet supported";
							return $raiseFormulaError; 
                        }
                    }
...
//                        echo 'Element '.$localeConstant.' is an Excel Constant<br />';
				...
				$raiseFormulaError = "Formula Error: Unexpected operator '$opCharacter'";
					return $raiseFormulaError; 
            } else {    // I don't even want to know what you did to get here
				$raiseFormulaError = "Formula Error: An unexpected error occured";
					return $raiseFormulaError; 
            }
            ...
            if ($index == strlen($formula)) {
                ...
                if ((isset(self::$operators[$opCharacter])) && ($opCharacter != '%')) {
					$raiseFormulaError = "Formula Error: Operator '$opCharacter' has no operands";
					return $raiseFormulaError; 
                } else {
                    break;
                }
...
//                    echo 'Element is an Intersect Operator<br />';
        ...
        while (($op = $stack->pop()) !== null) {    // pop everything off the stack and push onto output
            if ((is_array($op) && $op['value'] == '(') || ($op === '(')) {
					$raiseFormulaError = "Formula Error: Expecting ')'";
					return $raiseFormulaError; 				// if there are any opening braces on the stack, then braces were unbalanced
            }
            $output[] = $op;
        }
        return $output;
...
//                echo 'Token is a binary operator<br />';
                ...
                if (($operand2Data = $stack->pop()) === null) {
					$raiseFormulaError = 'Internal error - Operand value missing from stack';
					return $raiseFormulaError; 
                }
                    ...
                    case ':':            //    Range
                        ...
                        if ($sheet1 == $sheet2) {
                            ...
                            if ($pCellParent !== null) {
								...
								$raiseFormulaError = 'Unable to access Cell Reference';
								return $raiseFormulaError; 
                            }
...
//                        We can't access the range, so return a REF error
                        ...
                        if ($matches[2] > '') {
                            ...
                            if ((strpos($matches[2], '[') !== false) || (strpos($matches[2], ']') !== false)) {
								...
								$raiseFormulaError = 'Unable to access External Workbook';
								return $raiseFormulaError;
                            }
                            $matches[2] = trim($matches[2], "\"'");
...
//                    echo 'Named Range is '.$namedRange.'<br />';
					...
					$raiseFormulaError = "undefined variable '$token'";
								return $raiseFormulaError;
                }
            }
        }
        ...
        if ($stack->count() != 1) {
			$raiseFormulaError = 'internal error';
								return $raiseFormulaError;
        }
        $output = $stack->pop();
        $output = $output['value'];
