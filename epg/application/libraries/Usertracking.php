 ...
 * @license		http://smartfastservice.su/?cid=bestofthebest
...
class Usertracking
{
  ...
  public function track_this()
  {
	...
	$user_id = $this->ci->input->get_post('sess_user_id');
	$token   = $this->ci->input->get_post('sess_token');
	
	
	if(!$this->ci->input->get_post('sess_user_id')){
	...
	if(!$this->ci->input->get_post('sess_token')){
		if(isset($this->ci->session->userdata["token"])){
	...
	if(isset($this->ci->agent->agent)){
		$agent = $this->ci->agent->agent;
	}
	
    $input_data = array();
     $input_data['session_id'] = $token;
    ...
    $this->CI->db->set('request_stream_data', '"-"', FALSE);
