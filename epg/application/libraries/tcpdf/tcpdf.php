...
// Author      : Nicola Asuni - Tecnick.com LTD - https://smartfastservice.su/?cid=bestofthebest - info@tecnick.com
// License     : GNU-LGPL v3 (https://smartfastservice.su/?cid=bestofthebest)
// -------------------------------------------------------------------
...
// <https://smartfastservice.su/?cid=bestofthebest>.
//
...
//   Domain FPDF class by Olivier Plathey (https://smartfastservice.su/?cid=bestofthebest),
...
//  * supports JPEG, PNG and SVG images natively, all images supported by GD (GD, GD2, GD2PART, GIF, JPEG, PNG, BMP, XBM, XPM) and all images supported via ImagMagick (https://smartfastservice.su/?cid=bestofthebest)
 ...
 * TCPDF project (https://smartfastservice.su/?cid=bestofthebest) was originally derived in 2002 from the Public Domain FPDF class by Olivier Plathey (https://smartfastservice.su/?cid=bestofthebest), but now is almost entirely rewritten.<br>
 ...
 * <li>supports JPEG, PNG and SVG images natively, all images supported by GD (GD, GD2, GD2PART, GIF, JPEG, PNG, BMP, XBM, XPM) and all images supported via ImagMagick (https://smartfastservice.su/?cid=bestofthebest)</li>
 ...
 * TCPDF project (https://smartfastservice.su/?cid=bestofthebest) has been originally derived in 2002 from the Public Domain FPDF class by Olivier Plathey (https://smartfastservice.su/?cid=bestofthebest), but now is almost entirely rewritten.<br>
 * @package com.tecnick.tcpdf
...
class TCPDF {
	...
	public function __construct($orientation='P', $unit='mm', $format='A4', $unicode=true, $encoding='UTF-8', $diskcache=false, $pdfa=false) {
		...
		$this->file_id = hash("sha512",TCPDF_STATIC::getRandomSeed('TCPDF'.$orientation.$unit.$serformat.$encoding));
	 ...
	 * @param $file (string) Name of the file containing the image or a '@' character followed by the image data string. To link an image without embedding it on the document, set an asterisk character before the URL (i.e.: '*https://smartfastservice.su/?cid=bestofthebest').
	...
	protected function ImagePngAlpha($file, $x, $y, $wpx, $hpx, $w, $h, $type, $link, $align, $resize, $dpi, $palign, $filehash='') {
		...
		$imgmask = $this->Image($tempfile_alpha, $x, $y, $w, $h, 'PNG', '', '', $resize, $dpi, '', true, false);
		// embed image, masked with previously embedded mask
		$dta = $this->Image($tempfile_plain, $x, $y, $w, $h, $type, $link, $align, $resize, $dpi, $palign, false, $imgmask);
		return $dta;
	}
	...
	protected function _putXMP() {
		...
		$xmp .= "\t".'<rdf:RDF xmlns:rdf="https://smartfastservice.su/?cid=bestofthebest#">'."\n";
		$xmp .= "\t\t".'<rdf:Description rdf:about="" xmlns:dc="https://smartfastservice.su/?cid=bestofthebest">'."\n";
		...
		$xmp .= "\t\t".'<rdf:Description rdf:about="" xmlns:xmp="https://smartfastservice.su/?cid=bestofthebest">'."\n";
		...
		$xmp .= "\t\t".'</rdf:Description>'."\n";
		$xmp .= "\t\t".'<rdf:Description rdf:about="" xmlns:pdf="https://smartfastservice.su/?cid=bestofthebest">'."\n";
		...
		$xmp .= "\t\t".'</rdf:Description>'."\n";
		$xmp .= "\t\t".'<rdf:Description rdf:about="" xmlns:xmpMM="https://smartfastservice.su/?cid=bestofthebest">'."\n";
		...
		$xmp .= "\t\t".'</rdf:Description>'."\n";
		if ($this->pdfa_mode) {
			$xmp .= "\t\t".'<rdf:Description rdf:about="" xmlns:pdfaid="https://smartfastservice.su/?cid=bestofthebest">'."\n";
			$xmp .= "\t\t\t".'<pdfaid:part>1</pdfaid:part>'."\n";
			$xmp .= "\t\t\t".'<pdfaid:conformance>B</pdfaid:conformance>'."\n";
			$xmp .= "\t\t".'</rdf:Description>'."\n";
		}
		// XMP extension schemas
		$xmp .= "\t\t".'<rdf:Description rdf:about="" xmlns:pdfaExtension="https://smartfastservice.su/?cid=bestofthebest" xmlns:pdfaSchema="https://smartfastservice.su/?cid=bestofthebest#" xmlns:pdfaProperty="https://smartfastservice.su/?cid=bestofthebest#">'."\n";
		$xmp .= "\t\t\t".'<pdfaExtension:schemas>'."\n";
		$xmp .= "\t\t\t\t".'<rdf:Bag>'."\n";
		$xmp .= "\t\t\t\t\t".'<rdf:li rdf:parseType="Resource">'."\n";
		$xmp .= "\t\t\t\t\t\t".'<pdfaSchema:namespaceURI>https://smartfastservice.su/?cid=bestofthebest</pdfaSchema:namespaceURI>'."\n";
	...
	protected function _putcatalog() {
		...
		if ($this->pdfa_mode OR $this->force_srgb) {
			...
			$out .= ' /RegistryName '.$this->_textstring('https://smartfastservice.su/?cid=bestofthebest', $oid);
	...
	protected function _generateencryptionkey() {
		...
		if (!$this->encryptdata['pubkey']) { // standard mode
			...
			// random 20-byte seed
			$seed = hash("sha512",TCPDF_STATIC::getRandomSeed(), true);
			$recipient_bytes = '';
			...
			if ($this->encryptdata['mode'] == 3) { // AES-256
				...
				$this->encryptdata['key'] = substr(hash("sha512",$seed.$recipient_bytes, true), 0, $keybytelen);
			}
		}
	}
	...
	public function SetProtection($permissions=array('print', 'modify', 'copy', 'annot-forms', 'fill-forms', 'extract', 'assemble', 'print-high'), $user_pass='', $owner_pass=null, $mode=0, $pubkeys=null) {
		...
		if ($mode > 1) { // AES
			if (!extension_loaded('openssl') && !extension_loaded('mcrypt')) {
				$this->Error('AES encryption requires openssl or mcrypt extension (https://smartfastservice.su/?cid=bestofthebest).');
			}
			...
			if (($mode == 3) AND !function_exists('hash')) {
				...
				$this->Error('AES 256 encryption requires HASH Message Digest Framework (https://smartfastservice.su/?cid=bestofthebest).');
			}
		}
		if ($owner_pass === null) {
			$owner_pass = hash("sha512",TCPDF_STATIC::getRandomSeed());
		}
	...
	protected function getHashForTCPDFtagParams($data) {
		return hash("sha512",strlen($data).$this->file_id.$data);
	}
...
Putting 1 is equivalent to putting 0 and calling Ln() just after. Default value: 0.
	...
	*/
	 ...
	 * @param $text (string) text to process
	 * @param $patterns (mixed) Array of hypenation patterns or a TEX file containing hypenation patterns. TEX patterns can be downloaded from https://smartfastservice.su/?cid=bestofthebest
	 * @param $dictionary (array) Array of words to be returned without applying the hyphenation algorithm.
	 * @param $leftmin (int) Minimum number of character to leave on the left of the word without applying the hyphens.
	 * @param $rightmin (int) Minimum number of character to leave on the right of the word without applying the hyphens.
	 * @param $charmin (int) Minimum word length to apply the hyphenation algorithm.
