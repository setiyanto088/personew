...
class Audience_model extends CI_Model {
	...
	public function __construct()
	{
	 ...
	 function listsearch($sprog,$sdate,$scha,$srole){  	
        $query2 = "SELECT PROGRAM, START_PROGRAM FROM `M_SUMMARY_AUDIENCE_N_FTA_MAR18`
	 ...
	 function list_program($param){  	
        $query2 = "SELECT PROGRAM FROM `M_SUMMARY_AUDIENCE_N_FTA_MAR18`
  			WHERE DATE_FORMAT(`DATE`,'%d/%m/%Y') BETWEEN '".$param['date']."' AND '".$param['date2']."' 
	 ...
	 public function get_channel(){       
		 ...
		 $sql = "SELECT CHANNEL_CIM AS CHANNEL_CIM, CHANNEL_CDR AS CHANNEL_CDR FROM `P_CHANNEL_ADS_USEETV` C
        WHERE C.`FLAG_TV` = 0 AND FTAiST = 1 
	...
	public function list_profile() {
      $query = 'SELECT id, name FROM t_profiling_ub WHERE STATUS="1" AND flag = 1';  		
  ...
  public function current_date() {
	...
	public function get_userid($data) {
      ...
      return $sql->result_array();	   
		...
		$program_s[0] = $params['programsss'];
	$program_s[1] = $params['programsss'];
	
	
			if($program_s[0] == "ALL" ){
				
				$sql = "SELECT FIELD,SEGMENT,SUM(VIEWERS) AS VIEWERS,AVG(VIEWERS) VIEWERS_DAY FROM(SELECT `DATE`,FIELD, SEGMENT,VIEWERS  FROM `M_SUMMARY_AUDIENCE_N_FTA_MAR18`
				...
				)G GROUP BY SEGMENT ORDER BY VIEWERS DESC
				LIMIT 15";
	...
	$program_s[1] = $params['programsss'];
			...
			if($program_s[0] == "ALL" ){
				...
				$sql2 = "SELECT FIELD,SEGMENT,SUM(VIEWERS) AS VIEWERS,AVG(VIEWERS) VIEWERS_DAY FROM(SELECT `DATE`,FIELD, SEGMENT,VIEWERS  FROM `M_SUMMARY_AUDIENCE_N_FTA_MAR18`
				WHERE `DATE` BETWEEN '".$params['start_date']."' AND '".$params['end_date']."' 
				AND CHANNEL = '".$params['channel']."'
				AND PROGRAM = 'ALL'
				AND SEGMENT <> ''
	...
	public function list_audience($params = array()) {	
		...
		$program_s[1] = $params['programsss'];
			...
			if($program_s[0] == "ALL" ){
				...
				$sql2 = "
				SELECT FIELD,SEGMENT,SUM(VIEWERS) AS VIEWERS,AVG(VIEWERS) VIEWERS_DAY FROM(SELECT `DATE`,FIELD, SEGMENT,VIEWERS  FROM `M_SUMMARY_AUDIENCE_N_FTA_MAR18`
				WHERE `DATE` BETWEEN '".$params['start_date']."' AND '".$params['end_date']."' 
				AND CHANNEL = '".$params['channel']."'
				AND PROGRAM = '".$program_s[0]."'
				AND SEGMENT <> ''
