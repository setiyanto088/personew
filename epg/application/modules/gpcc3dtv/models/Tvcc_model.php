...
ORDER BY `DATE`,M1
	...
	if($params['program'] == 'ALL'){
		...
		$start_time = explode(' ',$arr_p[1]);
				...
				$name_tbs_new = strtoupper(date_format(date_create($arr_dates),"Ym"));  
			...
			if($params['daypart'] == "all"){
				...
				$dpart = explode('-',$params['daypart']);
				$day_w = "AND formatDateTime(USER_BEGIN_SESSION,'%H:%M:%S') BETWEEN '".$dpart[0]."' AND '".$dpart[1]."' ";
			}
			...
			if($params['channel'] == "0"){
				...
				$cin = "";
				foreach($chnsd as $channel_f){
					$cin = $cin."".$channel_f.",";
				}
				
				$new_cin = substr($cin, 0, -1);
				
				$channel_w = " AND CHANNEL IN (".$new_cin.") ";
			}
			
			if($params['datap']	== 'audience'){
				$as = "COUNT(DISTINCT(CARDNO))";
			}else if($params['datap']	== 'total_views'){
				$as = "COUNT(CARDNO)";
			}else{
				$as = "SUM(DURASI_PROGRAM)";
			}
				
			$sql = " 
				SELECT ".$as." AS VIEWERS, REPLACE(REPLACE(`GENRE_PROGRAM`, '\r', ''), '\n', '') AS GENRE, `DATE` 
				FROM `CDR_EPG_ALL_SESSION_HARIAN_".$name_tbs_new."_N`
				WHERE `DATE` BETWEEN '".$params['start_date']."' AND '".$params['end_date']."' 
				".$day_w."
				".$channel_w."
				GROUP BY REPLACE(REPLACE(`GENRE_PROGRAM`, '\r', ''), '\n', ''),`DATE`
				ORDER BY `DATE`,REPLACE(REPLACE(`GENRE_PROGRAM`, '\r', ''), '\n', '')
				";
  	...
  	function listgenre(){  	
        $query2 = " SELECT * FROM GENRE_PROGRAM_LIST ORDER BY GENRE ";       
	...
	public function list_charttvcc($params = array(),$chnsd, $datefix) {	
	
		$name_tbs_new = strtoupper(date_format(date_create($datefix['start_date']),"Ym"));  
		...
		$db = $this->clickhouse->db();
			...
			$sql = " 
				SELECT ".$as." AS VIEWERS, REPLACE(REPLACE(`GENRE_PROGRAM`, '\r', ''), '\n', '') AS GENRE, `DATE` 
				FROM `CDR_EPG_ALL_SESSION_HARIAN_".$name_tbs_new."_N`
				WHERE `DATE` BETWEEN '".$datefix['start_date']."' AND '".$datefix['end_date']."' 
				".$day_w."
				".$channel_w."
				GROUP BY REPLACE(REPLACE(`GENRE_PROGRAM`, '\r', ''), '\n', ''),`DATE`
				ORDER BY `DATE`,REPLACE(REPLACE(`GENRE_PROGRAM`, '\r', ''), '\n', '')
