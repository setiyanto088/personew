  	...
  	public function list_migration($params = array()){		
		...
		$query = 	"
					SELECT COUNT(*) AS jumlah FROM (
						SELECT A.*, B.CHANNEL_NAME_PROG FROM `CHANNEL_EVALUATION` A
								LEFT JOIN (SELECT CHANNEL_NAME_PROG,CHANNEL_NUMBER,QUALITY,CHANNEL_REG FROM `CHANNEL_PARAM_FINAL_EVAL` WHERE 
								`F2A_STATUS` IN (0,-99) AND CHANNEL_NAME NOT LIKE '% HD%'
								GROUP BY CHANNEL_NAME_PROG,CHANNEL_NUMBER,QUALITY,CHANNEL_REG ) B
 ...
 $out		= array();
		  ...
		  $total 			= $result[0]['jumlah'];
            ...
            $limit_data = $params['limit'] ;
				...
				$query2 = 	"
						SELECT A.*, B.* FROM `CHANNEL_EVALUATION` A
								LEFT JOIN (SELECT CHANNEL_NAME_PROG,CHANNEL_NUMBER,QUALITY,PACKAGES,CATEGORY,GENRE,CHANNEL_REG FROM `CHANNEL_PARAM_FINAL_EVAL` WHERE 
								`F2A_STATUS` IN (0,-99) AND CHANNEL_NAME NOT LIKE '% HD%'
								GROUP BY CHANNEL_NAME_PROG,CHANNEL_NUMBER,QUALITY,PACKAGES,CATEGORY,GENRE,CHANNEL_REG ) B
								ON A.CHANNEL_NAME = B.`CHANNEL_NAME_PROG`
								WHERE A.`PERIODE` = '".$params['profiles']."'
								ORDER BY `RANK`
