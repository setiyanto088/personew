...
class Createprofileu_model extends CI_Model {
	...
	public function list_profile($params = array()) {					
		...
		$sql 	= 'SELECT  *,a.id as idprofile  FROM t_profiling_ub a WHERE (STATUS = 1 OR STATUS = 3) AND user_id_profil IN('.$params['iduser'].',0) '.$filter.'
		...
		free_result($this->db->conn_id);
			...
			$total_filtered = $this->db->query('SELECT COUNT(a.id) as total_filtered FROM t_profiling_ub a where (status = 1 or status = 3) AND user_id_profil IN('.$params['iduser'].',0)')->row_array();
			$total = $this->db->query('SELECT COUNT(a.id) as total FROM t_profiling_ub a where (status = 1 or status = 3) AND user_id_profil IN('.$params['iduser'].',0)')->row_array();
...
public function create1($data) {
  ...
  public function insert_pid_partial($list,$periode_list){
	...
	public function inset_queue2($list_id,$periode_list,$command){
		...
		$script_check_date = "	INSERT INTO JOBS_QUEUE VALUES('".$list_id."','".$command."',NULL,NULL,2,'".date("Y-m-d H:i:s")."',15,NULL,NULL,2,'')  ";	
								
		$sql	= $this->db->query($script_check_date);
		$this->db->close();	
		
		$query2 = "UPDATE t_profiling_ub SET status_job = 1, date_process = '".date('Y-m-d H:i:s')."' WHERE `id` = ".$list_id;
