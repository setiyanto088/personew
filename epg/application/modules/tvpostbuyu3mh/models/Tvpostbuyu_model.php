...
class Tvpostbuyu_model extends CI_Model {
	...
	public function get_bulan(){
		...
		$query = "SELECT DISTINCT DATE_FORMAT(`DATE`, '%M') bulan FROM PTV_CIM_RATING_DTV2 order by DATE_FORMAT(`DATE`, '%m')";
	...
	public function get_grp($periode){
		$tgl = explode('-', $periode);
		$tahun = $tgl[0];
		...
		$db = $this->clickhouse->db();
		$query = "SELECT COUNT(CARDNO) AS grp FROM PTV_DETAIL_LOGPROOF_DTV2 WHERE formatDateTime(toDate(TANGGAL), '%Y-%m') = '".$tahun."-".$bulan."'";			
		$result = $db->select($query);
		return $result->rows();	   
