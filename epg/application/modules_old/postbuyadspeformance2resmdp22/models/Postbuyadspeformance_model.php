...
class Postbuyadspeformance_model extends CI_Model {
  ...
  public function current_date() {
	...
	public function _get_filter_adspeformance_pr($params = array()) {		
      ...
      if($params['start_date'] <> NULL){
           $l_where_clause_startdate = 'AND A.`DATE`  >= "'.$params['start_date'].'" ';
      } else {
          $l_where_clause_startdate = ''; 
      }
       
      if($params['end_date'] <> NULL){
           $l_where_clause_enddate = ' AND A.`DATE`  <= "'.$params['end_date'].'" ';
      } else {
          $l_where_clause_enddate = ''; 
      }
		...
		if($params['kategori'] == 'All'){
		  ...
		  if($params['kategoriby'] == 'null' || $params['kategoriby'] == ''  ){
			  ...
			  if($params['kategoriby'] == "PO_NUMBER"){
				  $l_where_clause_kategori = ' AND REPLACE('.$params['kategoriby'].', "\r", "") = "'.str_replace("*","'",$params['kategori']).'" ';
			  } else {
				  $l_where_clause_kategori = ' AND '.$params['kategoriby'].' = "'.str_replace("*","'",$params['kategori']).'" ';
			  }
		  }
		 }
      ...
      if($params['chnl'] == 'null' || $params['chnl'] == '0' || $params['chnl'] == ''){
	   ...
	   if($params['get_program_a'][0] == 'all' || $params['get_program_a'][0] == '0' || $params['get_program_a'][0] == ''){
           $l_where_clause_programD = ''; 
       } else {
          if($params['get_program_a'][0] == 'all'){
              $l_where_clause_programD = '';
           } else {
 			$cinDS = "";
      
			foreach($params['get_program_a'] as $channel_fDS){
				$cinDS = $cinDS."'".$channel_fDS."',";
			}
			
			
			$new_cinDS = substr($cinDS, 0, -1);
			

            $l_where_clause_programD = ' AND A.PROGRAM IN ('.$new_cinDS.') ';
	...
	SELECT A.`DATE`,A.`CHANNEL`,A.`PROGRAM`,A.`SPLIT_MINUTES`,A.`START_TIME`,A.`END_TIME`,A.`DURATION`,A.`DURATION`,A.`NAMA_BRAND`,
			...
			DAYNAME(A.DATE) DAYENAME,WEEK(A.DATE) WEEKNAME FROM (
			...
			AND A.NAMA_BRAND = B.NAMA_BRAND AND A.PO_NUMBER = B.PO_NUMBER
			ORDER BY A.CHANNEL, A.`DATE`, A.START_TIME
     ";	
	 
       
      $query2		= $this->db2->query($sql22);
  ...
  SELECT  CHANNEL,PROGRAM, `DATE` AS TANGGAL, START_TIME, VIEWERS AS TOTAL_VIEW, 1 AS SPOT 
	  ...
	  SELECT COUNT(*) AS jumlah FROM (
		...
		SELECT A.`DATE`,A.`CHANNEL`,A.`PROGRAM`,A.`SPLIT_MINUTES`,A.`START_TIME`,A.`END_TIME`,A.`DURATION`,A.`NAMA_BRAND`,
			...
			DAYNAME(A.DATE) DAYENAME,WEEK(A.DATE) WEEKNAME FROM (
				...
				".$l_where_clause_product."
				".$l_where_clause_program."
			) B ON A.DATE = B.DATE AND A.CHANNEL = B.CHANNEL AND A.PROGRAM = B.PROGRAM AND A.START_TIME = B.START_TIME AND A.PO_NUMBER = B.PO_NUMBER
	...
	public function _get_filter_adspeformance2_ss($params = array()) {		
      ...
      $sql2 = "
			SELECT A.`DATE`,A.`CHANNEL`,A.`PROGRAM`,A.`SPLIT_MINUTES`,A.`START_TIME`,A.`END_TIME`,A.`DURATION`,A.`DURATION`,A.`NAMA_BRAND`,
			A.`SLOT_NAME`,A.`PRICE`,A.`NET_PRICE`,A.`ADVERTISER`,A.`AGENCY`,A.`PO_NUMBER`,A.`STATUS`,A.`PROVIDER`,
			B.`VIEWERS`,B.`ALL_VIEWERS`,B.`UNIVERSE`,B.`TVS`,B.`TVR`,B.`ID_PROFILE`,B.`REACH`,
			DAYNAME(A.DATE) DAYENAME,WEEK(A.DATE) WEEKNAME FROM (
				SELECT A.* FROM PTV_CIM_RATING_RES A 
				 JOIN `LOGPROOF_RES_FULL_STEP2` B 
				 ON A.CHANNEL = B.CHANNEL 
			...
			AND A.NAMA_BRAND = B.NAMA_BRAND
			GROUP BY A.CHANNEL, A.`DATE`, A.START_TIME
			ORDER BY A.CHANNEL, A.`DATE`, A.START_TIME
			";	
	...
	public function get_filter_grandtotal_adspeformance($params = array()) {	
	  ...
	  if($params['get_product'] == 'null' || $params['kategoriby'] == ''|| $params['get_product'] == 'undefined'){
  		...
  		$sql2 = "
		SELECT CHANNEL, SUM(TOTAL_VIEW) AS VIEWS, SUM(SPOT) AS SPOT, SUM(TOTAL_VIEW)/SUM(SPOT) AS VG_VPS, SUM(PRICE) AS PRICE FROM (
			SELECT CHANNEL, `DATE` AS TANGGAL, START_TIME, VIEWERS AS TOTAL_VIEW, 1 AS SPOT ,PRICE
			FROM (
				SELECT A.`DATE`,A.`CHANNEL`,A.`PROGRAM`,A.`SPLIT_MINUTES`,A.`START_TIME`,A.`END_TIME`,A.`DURATION`,A.`NAMA_BRAND`,
				A.`SLOT_NAME`,A.`PRICE`,A.`NET_PRICE`,A.`ADVERTISER`,A.`AGENCY`,A.`PO_NUMBER`,A.`STATUS`,A.`PROVIDER`,
				B.`VIEWERS`,B.`ALL_VIEWERS`,B.`UNIVERSE`,B.`TVS`,B.`TVR`,B.`ID_PROFILE`,B.`REACH`,
...
GROUP BY CHANNEL ORDER BY CHANNEL
	...
	public function _get_filter_adspeformance2_reach_all($params = array()){
      ...
      $sql 	= "
		 ...
		 SELECT ".$params['kategoriby'].",
  		 ...
  		 FROM (
			 ...
			 IF(COUNT(RESPID) >= 1,WEIGHT,0) REACH1,
			 IF(COUNT(RESPID) >= 2,WEIGHT,0) REACH2,
			 IF(COUNT(RESPID) >= 3,WEIGHT,0) REACH3,
			 IF(COUNT(RESPID) >= 7,WEIGHT,0) REACH7,
			 IF(COUNT(RESPID) >= 13,WEIGHT,0) REACH13,
			 IF(COUNT(RESPID) >= 21,WEIGHT,0) REACH21 FROM ".$tbls." A
			 WHERE 1=1 
			 ".$l_where_clause_startdate."
			 ".$l_where_clause_enddate."
			 ".$l_where_clause_kategori."
			 ".$l_where_clause_channel."
			 ".$l_where_clause_program."
			 GROUP BY RESPID, ".$params['kategoriby']."
  		 ) A 
      ...
      $sql 	= "
		...
		SELECT ".$params['kategoriby'].",
  		 ...
  		 (SUM(REACH21)/".$params['univ'].")*100 REACH_21
  		 FROM (
			 SELECT RESPID,COUNT(RESPID) CNT,A.WEIGHT, ".$params['kategoriby'].",
			 IF(COUNT(RESPID) >= 1,A.WEIGHT,0) REACH1,
			 IF(COUNT(RESPID) >= 2,A.WEIGHT,0) REACH2,
			 IF(COUNT(RESPID) >= 3,A.WEIGHT,0) REACH3,
			 IF(COUNT(RESPID) >= 7,A.WEIGHT,0) REACH7,
			 IF(COUNT(RESPID) >= 13,A.WEIGHT,0) REACH13,
			 IF(COUNT(RESPID) >= 21,A.WEIGHT,0) REACH21 FROM REACH_POSTBUY_RES_P22 A 
	  ...
	  ";
		...
		SELECT PRODUCT,
  		 ...
  		 (SUM(REACH21)/".$params['univ'].")*100 REACH_21
  		 FROM (
			 SELECT RESPID,COUNT(RESPID) CNT,A.WEIGHT, PRODUCT,
			 IF(COUNT(RESPID) >= 1,A.WEIGHT,0) REACH1,
			 IF(COUNT(RESPID) >= 2,A.WEIGHT,0) REACH2,
			 IF(COUNT(RESPID) >= 3,A.WEIGHT,0) REACH3,
			 IF(COUNT(RESPID) >= 7,A.WEIGHT,0) REACH7,
			 IF(COUNT(RESPID) >= 13,A.WEIGHT,0) REACH13,
			 IF(COUNT(RESPID) >= 21,A.WEIGHT,0) REACH21 FROM REACH_POSTBUY_RES_PRODUCT_P22 A 
			 JOIN `PROFILE_CARDNO_RES` B ON A.RESPID = B.CARDNO
			WHERE B.`ID_PROFILE` = ".$params['profile']."  
			 ".$l_where_clause_startdate."
			 ".$l_where_clause_enddate."
  ...
  public function _list_subkategori($params){
		...
		$sql 	= "SELECT DISTINCT(REPLACE(A.".$kategori.",\"'\",\"*\")) AS SUBCAT FROM PTV_CIM_RATING_RES A
