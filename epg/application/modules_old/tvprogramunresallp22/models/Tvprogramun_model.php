	...
	public function list_spot_by_program_all_bar_avg_alls_exp($daypart,$where,$periode,$pilihaudiencebar,$profile,$check,$start_date,$end_date,$order,$search,$survey) {
		...
		if($survey == '2022'){
			...
			if($profile == 1 || $profile == 0){
				...
				$prof_qr = '
							...
							SELECT `CHANNEL_NAME_PROG` AS channel,SUM(WEIGHT) AS VIEWERS2,SUM(WEIGHT_ALL) AS Spot2,PERIODE,"AUDIENCE" AS DT,0 AS IDPRO, STS FROM (
							SELECT DATE_FORMAT(BEGIN_PROGRAM,"%Y-%m-%d") HARIS,`CHANNEL_NAME_PROG`,A.RESPID, WEIGHT,WEIGHT_ALL, "'.$periode.'" PERIODE, 0 STS 
							FROM CDR_EPG_RES_ALL_STEP2_2022 A JOIN CHANNEL_PARAM_FINAL B
		...
		if($daypart == 'ALL' ){
			...
			if($profile == 0){
				...
				$qry_ends = '
					SELECT A.CHANNEL,DATE_FORMAT(`SPLIT_MINUTES`,"%Y-%m-%d") `DATE`, 
					...
					SELECT A.CHANNEL,DATE_FORMAT(`SPLIT_MINUTES`,"%Y-%m-%d") `DATE`, 
								...
								FROM (
									SELECT A.*,(A.TVR/B.TVR)*100 AS IDEX, (A.VIEWERS/A.UNIVERSE)*100 AS TVRR,(A.VIEWERS/A.ALL_VIEWS)*100 AS TVSS FROM (
										...
										AND DATE_FORMAT(A.SPLIT_MINUTES,"%H:%i:%s") BETWEEN "'.$params['start_time'].'" AND "'.$params['end_time'].'"  
									) A JOIN (
										SELECT * FROM `SUMMARY_PER_MINUTES_RES_V2` A
										WHERE A.`PROFILE_ID` = 0    
										'.$where.' '.$wh_chn.' 
										AND  A.`SPLIT_MINUTES` BETWEEN "'.$start_date.' 00:00:00" AND "'.$end_date.' 23:59:59"
										AND DATE_FORMAT(A.SPLIT_MINUTES,"%H:%i:%s") BETWEEN "'.$params['start_time'].'" AND "'.$params['end_time'].'"  
...
SELECT CHANNEL AS channel,VIEWERS AS Spot FROM M_SUM_TV_DASH_CHAN_DURATION_WEEK_PTV R WHERE 
	...
	public function list_spot_by_program_all2Ps_new_alls($field,$wheres,$params,$pilihprog,$profile) {
		...
		$where = " AND (A.PROGRAM LIKE '%".$params['searchtxt']."%' OR A.CHANNEL LIKE '%".$params['searchtxt']."%') AND TANGGAL = '".$params['periode']."'";
...
IF($periode == '2021-October' || $periode == '2021-September' || $periode == '2021-November'  || $periode == '2021-December' || substr($periode,0,4) == '2022'){

 	$query = 'SELECT COUNT(RESPID) jmlch FROM `URBAN_PROFILE_2021` WHERE tv_genre IS NOT NULL ';  	

}else{
	$query = 'SELECT COUNT(RESPID) jmlch FROM `HELIX_RES3` ';  	
}	 			
 		$sql	= $this->db2->query($query);
		$this->db2->close();
		$this->db2->initialize(); 
