...
class Createprofileu_model extends CI_Model {
	...
	public function list_profile($params = array()) {					
		...
		free_result($this->db2->conn_id);
			...
			$total_filtered = $this->db2->query('SELECT COUNT(a.id) as total_filtered FROM t_profiling_ub_res a where (status in (1,2,3)) AND user_id_profil IN('.$params['iduser'].',0)')->row_array();
			$total = $this->db2->query('SELECT COUNT(a.id) as total FROM t_profiling_ub_res a where (status = 1 or status = 3) AND user_id_profil IN('.$params['iduser'].',0)')->row_array();
...
public function create($data,$where,$grouping) {
	...
	$queryid = "SELECT COUNT(RESPID) AS people, SUM(WEIGHT) AS populasi,SUM(WEIGHT) AS populasi_all FROM `URBAN_PROFILE_P22` WHERE tv_genre IS NOT NULL AND WEIGHT <> 0 ".$where;
	
	
	$sqliCNT	= $this->db2->query($queryid);
				...
				$this->db2->query($sqlada);
						...
						$script_check ="SELECT TIMESTAMPDIFF(MONTH, '2022-12-01', '".date("Y-m-d")."') as DIFF ";
							...
							for($i=0;$i<14;$i++){
								...
								if($i < 3){
									$sql5 = "INSERT INTO M_MONTH_PROFILE_RES_P22 VALUES('".$i0."','','',".$id.",'2') ";
								}else{
									$sql5 = "INSERT INTO M_MONTH_PROFILE_RES_P22 VALUES('".$i0."','','',".$id.",'0') ";
								}
								$this->db2->query($sql5);
