...
class Tvpostbuyu_model extends CI_Model {
	...
	public function list_spot_by_date_all($ORDERS,$where,$periode,$user) {
		 
					
		 $db = $this->clickhouse->db();
		 $query = "SELECT ".$ORDERS." AS spot,`DATE` FROM PB_DASH_DAYS_RES
...
$result = $db->select($query);
	...
	public function list_spot_by_daytime_all($ORDERS,$where,$periode,$user) {
	 
						 
		$db = $this->clickhouse->db();				 
		 $query = "SELECT ".$ORDERS." AS spot,DAYPART FROM PB_DASH_DAYPART_RES 
						 WHERE PERIODE = '".$periode."' 
						 AND PROFILE_ID = '".$user."'
...
* FROM `PTV_DETAIL_LOGPROOF_DTV2` a
	...
	public function list_spot_by_program_all2n($segment,$ORDERS,$where,$periode,$user) {
 
				
			 $db = $this->clickhouse->db();
			 $query = "SELECT NAME, CHANNEL,".$ORDERS." as spots FROM PB_DASH_SEGMENT2_RES 
						 WHERE PERIODE = '".$periode."' 
						 AND SEGMENT = '".$segment."'
						 AND PROFILE_ID = '".$user."'
	...
	public function list_spot_by_program_all($segment,$ORDERS,$where,$periode,$user) {

 
						
								$db = $this->clickhouse->db();
			 $query = "SELECT NAME, ".$ORDERS." as spots FROM PB_DASH_SEGMENT_RES 
						 WHERE PERIODE = '".$periode."' 
						 AND SEGMENT = '".$segment."'
						  AND PROFILE_ID = '".$user."'
						ORDER BY toFloat32(".$ORDERS.")	 DESC
	...
	public function list_spot_by_chanel_all($ORDERS,$where,$periode,$user) {
 
						
						$db = $this->clickhouse->db();	
						 $query = "SELECT * FROM PB_DASH_CHANNEL_RES 
						 WHERE PERIODE = '".$periode."'
						AND PROFILE_ID = '".$user."'
						ORDER BY ".$ORDERS." DESC
						LIMIT 30"; 
...
AND DATE_FORMAT(STR_TO_DATE(TANGGAL,"%d/%m/%Y"), "%Y-%M") = "'.$periode.'" '.$where.' ';   
	...
	public function list_cost_alls_dtv($where,$periode,$user) {
		$db = $this->clickhouse->db();
		
		$query = "

		SELECT * FROM `PB_DASH_SPOTCOST_RES`
		WHERE 1=1  
		AND PROFILE_ID = '".$user."'
		AND PERIODE = '".$periode."' 
