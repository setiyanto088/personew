...
class Tvprogramun_model extends CI_Model {
	...
	public function get_last_day() {
		...
		$query = "SELECT MAX(`DATE`) last_day FROM `M_SUM_TV_DASH_DATE_PTV`";
	...
	public function get_active_audience($periode){
 		...
 		$query = "SELECT COUNT(DISTINCT(CARDNO)) VIEWERS FROM CARDNO_DAY_CHANNEL  
		WHERE TYPE_DATA = 'LIVE' AND `DAYS` BETWEEN '".$periode['start_date']."' and '".$periode['end_date']."'"	
		;
	...
	public function get_total_views($periode){
		...
		WHERE `DAYS` BETWEEN '".$periode['start_date']."' and '".$periode['end_date']."'" ;
...
SELECT CHANNEL AS channel,VIEWERS AS Spot FROM M_SUM_TV_DASH_CHAN_DURATION_WEEK_PTV R WHERE 
	...
	$query = 'SELECT COUNT(DISTINCT(CHANNEL)) jmlch,COUNT(DISTINCT(PROGRAM)) jmlpr FROM `PTV_M_SUM_TV_DASH_PROG_DAYS`
	WHERE DAYS BETWEEN "'.$periode['start_date'].'" AND "'.$periode['end_date'].'" AND ID_PROFILE = 0 ';  		 			
 		$sql	= $this->db2->query($query);
		$this->db2->close();
		$this->db2->initialize(); 
		return $sql->result_array();	   
	...
	$query = 'SELECT COUNT(DISTINCT(CHANNEL)) jmlch,COUNT(DISTINCT(PROGRAM)) jmlpr FROM `M_SUM_TV_DASH_PROG_TVOD_DAYS`
	WHERE DAYS BETWEEN "'.$periode['start_date'].'" AND "'.$periode['end_date'].'" AND ID_PROFILE = 0 AND `TIPE_FILTER` = "TVOD"';  		 			
 		$sql	= $this->db2->query($query);
		$this->db2->close();
		$this->db2->initialize(); 
		return $sql->result_array();	   
	...
	$query = 'SELECT COUNT(DISTINCT(CHANNEL)) jmlch,COUNT(DISTINCT(PROGRAM)) jmlpr FROM `M_SUM_TV_DASH_PROG_TVOD_DAYS`
	WHERE DAYS BETWEEN "'.$periode['start_date'].'" AND "'.$periode['end_date'].'" AND ID_PROFILE = 0 AND `TIPE_FILTER` = "ALL"';  		 			
 		$sql	= $this->db2->query($query);
		$this->db2->close();
		$this->db2->initialize(); 
		return $sql->result_array();	   
	...
	$query = 'SELECT COUNT(DISTINCT(PROGRAM)) jmlpr, COUNT(DISTINCT(CHANNEL)) jmlchn FROM `M_SUM_TV_DASH_PROG_TVOD_DAYS`
	WHERE `DAYS` BETWEEN "'.$periode['start_date'].'" and "'.$periode['end_date'].'" AND ID_PROFILE = 0 AND `TIPE_FILTER` = "ALL"';  		 			
 		$sql	= $this->db2->query($query);
		$this->db2->close();
		$this->db2->initialize(); 
	...
	public function get_active_audience_tvod($periode){
		...
		$query = "SELECT COUNT(DISTINCT(CARDNO)) VIEWERS FROM CARDNO_DAY_CHANNEL  
		WHERE TYPE_DATA = 'TVOD' AND `DAYS` BETWEEN '".$periode['start_date']."' and '".$periode['end_date']."' " ;
	...
	public function get_active_audience_all($periode){
		...
		$query = "
		SELECT SUM(VIEWERS) AS VIEWERS FROM (
		SELECT COUNT(DISTINCT(CARDNO)) VIEWERS,TYPE_DATA  FROM CARDNO_DAY_CHANNEL  
		WHERE  `DAYS` BETWEEN '".$periode['start_date']."' and '".$periode['end_date']."'
		GROUP BY TYPE_DATA
	...
	public function get_total_views_tvod($periode){
		...
		WHERE `DAYS` BETWEEN '".$periode['start_date']."' and '".$periode['end_date']."' AND TIPE_VIEW = 'TOTAL_VIEWS' AND TIPE_FILTER = 'TVOD' " ;
	...
	public function get_total_views_all($periode){
		...
		$query = " SELECT A.TOTAL_VIEWS+B.TOTAL_VIEWS AS TOTAL_VIEWS FROM (
			SELECT SUM(VIEWERS) as TOTAL_VIEWS FROM `M_SUM_TV_DASH_CHAN_TVOD_DAYS`
			WHERE `DAYS` BETWEEN '".$periode['start_date']."' and '".$periode['end_date']."' AND TIPE_VIEW = 'TOTAL_VIEWS' AND TIPE_FILTER = 'TVOD' 
		) A,(
			SELECT SUM(VIEWERS) as TOTAL_VIEWS FROM `M_SUM_TV_DASH_CHAN_VIEWERS_DAYS_PTV`
			WHERE `DAYS` BETWEEN '".$periode['start_date']."' and '".$periode['end_date']."'
		) B" ;
	...
	public function get_duration_tvod($periode){
		...
		WHERE `DAYS` BETWEEN '".$periode['start_date']."' and '".$periode['end_date']."' AND TIPE_VIEW = 'DURATION' AND TIPE_FILTER = 'TVOD' " ;
	...
	public function get_duration_all($periode){
		...
		$query = "SELECT A.DURATION+B.DURATION AS DURATION FROM (
			SELECT SUM(VIEWERS) as DURATION FROM `M_SUM_TV_DASH_CHAN_TVOD_DAYS`
			WHERE `DAYS` BETWEEN '".$periode['start_date']."' and '".$periode['end_date']."' AND TIPE_VIEW = 'DURATION' AND TIPE_FILTER = 'TVOD' 
		) A, (
			SELECT SUM(VIEWERS) as DURATION FROM `M_SUM_TV_DASH_CHAN_DURATION_DAYS_PTV`
			WHERE `DAYS` BETWEEN '".$periode['start_date']."' and '".$periode['end_date']."'
		) B" ;
