		...
		if($params['check'] == "True"){
				$wh_chn = ''; 
				 ...
				 $query = 	"	
					...
					#WHERE STR_TO_DATE(TANGGAL,'%Y-%M') BETWEEN STR_TO_DATE('".$params['tgl1mr']."','%Y-%M') and STR_TO_DATE('".$params['tgl1mr']."','%Y-%M')
					WHERE STR_TO_DATE(TANGGAL,'%Y-%M') BETWEEN STR_TO_DATE('".$params['tgl1mr']."','%Y-%M') AND STR_TO_DATE('".$params['tgl1mr']."','%Y-%M')
					GROUP BY CHANNEL
					...
					";
					 ...
					 $query2 = 	"	
					SELECT z.*, rank() over ( ORDER BY AUDIENCE DESC) AS Rangking FROM 
					( 
						SELECT A.CHANNEL, B.VIEWERS AS AUDIENCE, C.`VIEWERS` AS TVR, D.`VIEWERS` AS TVS, E.`VIEWERS` AS VIEWER, C.VIEWERS AS REACH FROM (
