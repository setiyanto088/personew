	...
	public function list_labels($params = array(),$arr_dates){
  	...
  	public function list_charttvcc_allall($params = array(),$arr_dates) {	
			$name_tb = strtoupper(date_format(date_create($arr_dates),"yM")); 
				...
				$sql = "
				SELECT 'ALL CHANNEL' AS CHANNEL,PROGRAM,SUM(VIEWERS) AS VIEWERS,'ALL PROGRAM'  AS FULL_PROG, SPLIT_MINUTES, DATE_FORMAT(SPLIT_MINUTES,'%H:%i') AS MINUTES 
				FROM `RATING_PER_MINUTES_".$name_tb."` 
				WHERE PROFILE_ID = 0 
				AND CHANNEL IN ('Al Jazeera','Bloomberg','Channel News Asia','CNBC Asia','CNN International','DW TV','Euronews','France 24','SEA Today','TRT World','TVBS News','CNBC','TV One','CNN Indonesia','Metro TV','Kompas TV','TVRI','Berita Satu','TVRI','iNews','IDX Channel','MNC News','CNBC Indonesia')
				AND DATE_FORMAT(SPLIT_MINUTES,'%Y-%m-%d') = '".$arr_dates."'
				GROUP BY SPLIT_MINUTES
				ORDER BY SPLIT_MINUTES
				";
