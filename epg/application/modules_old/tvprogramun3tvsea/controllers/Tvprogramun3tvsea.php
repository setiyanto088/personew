		...
		$in_month = '';
 		if($params['week'] == 'All'){
		   ...
		   $it1 = 2;
			...
			$query_qr = "SELECT z.*, rank() over ( ORDER BY TOTAL DESC,PROGRAM DESC) AS Rangking FROM 
			( SELECT CHN.CHANNEL,CHN.PROGRAM, ";
			$week_in = "";
			$join_left = "";
			$ri = 1;
			$th_tb = "";
			...
			$query_qr = $query_qr."
					A1.VIEWERS AS TOTAL 
					FROM (
						SELECT CHANNEL,PROGRAM FROM `".$tbt_2."`
		...
		if($this_day == "All"){
			...
			if($tipe_filter == 'live'){
				...
				IF($type == 'AUDIENCE'){
					...
					$tbt = 'M_SUM_TV_DASH_CHAN_VIEWERS_PTV';
					$query_qr2 = $query_qr2.''.$week_in2.' FROM (
								SELECT CHANNEL,'.$drag.'(IF(CHANNEL = "SEA TODAY",CEIL(VIEWERS*1.87),VIEWERS)) AS VIEWERS FROM '.$tbt.'
							...
							$query_qr2 = $query_qr2.''.$week_in2.' FROM (
										...
										SELECT A.CHANNEL,A.VIEWERS+IF(B.VIEWERS IS NULL,0,B.VIEWERS) VIEWERS FROM (
											...
											GROUP BY CHANNEL
													...
													SELECT CHANNEL,'.$drag.'(IF(CHANNEL = "SEA TODAY",CEIL(VIEWERS*1.87),VIEWERS)) AS VIEWERS FROM M_SUM_TV_DASH_CHAN_VIEWERS_PTV
													WHERE SUBSTR(`TANGGAL`,1,4) = "'.$first_day.'"
					...
					IF($ri == 1){
						...
						$join_left = $join_left." 
							LEFT JOIN (	
								SELECT z.*, rank() over ( ORDER BY VIEWERS DESC,CHANNEL DESC) AS Rangking FROM 
									( 
										SELECT `CHANNEL`,".$drag."(".$vv.") AS VIEWERS,`TANGGAL`,`WEEK`,`ID_PROFILE` FROM ".$tbt2F."
										WHERE SUBSTR(TANGGAL,1,4) = '".$wkwk['YEAR']."' AND `WEEK` = ".($wkwk['WEEK']-$w_week)."
										".$where."
										GROUP BY CHANNEL
									) z ORDER BY Rangking
			...
			if($tipe_filter == "live"){ 
				...
				$query_qr = $query_qr." A1.VIEWERS AS TOTAL
					FROM (
						SELECT DISTINCT(CHANNEL) FROM ".$tbt2F."
						WHERE SUBSTR(TANGGAL,1,4) = '".$data['monthdt'][0]['YEAR']."' AND `WEEK` IN (".$week_in.")
						AND CHANNEL IS NOT NULL
						".$where."
					) CHN  ".$join_left." 
					WHERE 1=1  AND A1.Rangking is not null ORDER BY  A1.Rangking  "; 
	...
	public function audiencebar_by_channel_export2(){
	   ...
	   'AA','AB','AC','AD','AE','AF','AG','AH','AI','AJ','AK'];
		   ...
		   $objPHPExcel->setActiveSheetIndex(0)
			...
			foreach($data['monthdt'] as $wkwk){
 				$query_qr = $query_qr." IF(A".$ri.".`VIEWERS` is null,0,A".$ri.".`VIEWERS`) AS WE".$ri.",A".$ri.".CHANNEL AS CHANNEL".$ri.",";
				$week_in = $week_in.''.($wkwk['WEEK_MONTH']-1).',';
							...
							LEFT JOIN (	
								SELECT z.*, rank() over ( ORDER BY VIEWERS DESC,CHANNEL DESC) AS Rangking FROM 
									( 
										...
										WHERE SUBSTR(TANGGAL,1,4) = '".$wkwk['YEAR']."' AND `WEEK` = ".($wkwk['WEEK_MONTH']-1)."
										".$where."
										GROUP BY CHANNEL
									) z ORDER BY Rangking
								) A".$ri." ON CHN.CHANNEL = A".$ri.".CHANNEL 
