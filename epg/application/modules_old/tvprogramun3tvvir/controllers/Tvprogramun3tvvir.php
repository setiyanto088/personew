		   ...
		   //PRINT_R($data);die;
			...
			//echo $query_qr;die;
				...
				foreach($data['monthdt'] as $wkwk){
					...
					//$query_qr = $query_qr."A".$ri.".`VIEWERS` AS WE".$ri.",";
					$query_qr = $query_qr." IF(A".$ri.".`VIEWERS` is null,0,A".$ri.".`VIEWERS`) AS WE".$ri.",A".$ri.".`CHANNEL` AS CHANNEL".$ri.", A".$ri.".`PROGRAM` AS PROGRAM".$ri.",";
					$week_in = $week_in.''.(($wkwk['WEEK']-$w_week+1)).',';
					...
					IF($tipe_filter == 'TVOD'){
						...
						IF($ri == 1){
							$join_left = $join_left." 
									...
									SELECT z.*, rank() over ( ORDER BY VIEWERS DESC,CHANNEL DESC) AS Rangking FROM 
													( 
										SELECT A.`CHANNEL`, ".$drag."(A.VIEWERS)+".$drag."(B.VIEWERS) VIEWERS,A.`TANGGAL`,A.`WEEK`,A.`ID_PROFILE` FROM `M_SUM_TV_DASH_CHAN_TVOD_WEEKS` A
										JOIN ".$tbt2F." B ON A.CHANNEL = B.CHANNEL AND A.`TANGGAL` = B.`TANGGAL` AND A.WEEK = B.WEEK
										WHERE SUBSTR(A.TANGGAL,1,4) = '".$wkwk['YEAR']."'   AND  A.`WEEK` = ".((($wkwk['WEEK']-$w_week)))."
										AND A.TIPE_FILTER = 'TVOD' AND A.TIPE_VIEW = '".$tipe_tvod."'
										".$where2s."
										GROUP BY SUBSTR(A.TANGGAL,1,4),A.CHANNEL,A.`WEEK`
									) z ORDER BY Rangking
									) A".$ri." ON CHN.Rangking = A".$ri.".Rangking 
