...
class CSRF_Protection
{
			  ...
			  private static $token_name = 'csrf_pmis_secure';
 ...
 */
			public function generate_token()
			{
			  ...
			  if ($this->CI->session->userdata(self::$token_name) === FALSE)
			  {
				...
				$hash = '"'. $this->security->get_csrf_token_name().'"';
				self::$token = hash("sha512",uniqid() . microtime() );
			...
			public function inject_tokens()
			{
			  ...
			  // Inject into form
				$hashCode =  $this->CI->security->get_csrf_hash();
				$hashname =  $this->CI->security->get_csrf_token_name();
			     $output = preg_replace('/(<(form|FORM)[^>]*(method|METHOD)="(post|POST)"[^>]*>)/',
                           '$0<input type="hidden" name="' . $hashname . '" value="' . $hashCode . '" id="'.$hashname.'">', 
                           $output);
			 
			 
			  // Inject into <head>
			  $output = preg_replace('/(<\/head>)/',
									 '<meta name="csrf-name" content="' . $hashname . '">' . "\n" . '<meta name="csrf-token" content="' . $hashCode . '">' . "\n" . '$0', 
									 $output);
