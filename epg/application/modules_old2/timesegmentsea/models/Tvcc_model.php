...
ORDER BY `DATE`,M1
	...
	$db = $this->clickhouse->db();
			$name_tb = strtoupper(date_format(date_create($arr_dates),"yM")); 
				...
				WHERE PROFILE_ID = '0' 
				AND CHANNEL IN ('AL JAZEERA','BLOOMBERG','CHANNEL NEWS ASIA','CNBC','CNN INTERNATIONAL','DW TV','EURONEWS','FRANCE 24','SEA TODAY','TRT WORLD','TVBS NEWS','CNBC','TV ONE','CNN INDONESIA','METRO TV','KOMPAS TV','TVRI','BERITA SATU','TVRI','INEWS','IDX','MNC NEWS','CNBC INDONESIA')
	...
	$name_tb = strtoupper(date_format(date_create($arr_dates),"yM"));  
		$db = $this->clickhouse->db();
			...
			$sql = "
				SELECT *,CONCAT(PROGRAM,'||',formatDateTime(SPLIT_MINUTES,'%Y-%m-%d'),' ',START_TIME) AS FULL_PROG, formatDateTime(SPLIT_MINUTES,'%H:%M') AS MINUTES FROM `RATING_PER_MINUTES_".$name_tb."`
				WHERE PROFILE_ID = 0 
