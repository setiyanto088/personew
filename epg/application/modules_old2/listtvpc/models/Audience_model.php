...
class Audience_model extends CI_Model {
	...
	public function __construct()
	{
     ...
     $this->db->query($sql);
  	...
  	public function list_channel() {
		...
		$query = "SELECT DISTINCT B.`CHANNEL_NAME_PROG` AS CHANNEL_CIM,B.`CHANNEL_NAME_PROG` AS CHANNEL_CDR FROM  `CHANNEL_PARAM_FINAL` B WHERE F2A_STATUS <> 1 ORDER BY CHANNEL_NAME_PROG ";
		...
		public function channelsearch($wh,$id) {
		
		$query = "SELECT DISTINCT B.`CHANNEL_NAME_PROG` AS CHANNEL_CIM,B.`CHANNEL_NAME_PROG` AS CHANNEL_CDR FROM  `CHANNEL_PARAM_FINAL` B WHERE F2A_STATUS <> 1 AND CHANNEL_NAME_PROG  LIKE '%".$wh."%' ORDER BY CHANNEL_NAME_PROG ";
  ...
  public function channel_set($preset,$userid){ 
	...
	public function tree1(){
		...
		$query = "SELECT A.*,B.`FIELD` AS PARENTFIELD, B.DEFAULT AS PARENTDEFAULT FROM `TREE_PROFILING_RES` A LEFT JOIN `TREE_PROFILING_RES` B
		ON A.`PARENTID` = B.`ID`
	...
	public function get_prof($ssss){
		...
		$query = "
			SELECT A.*,B.`FIELD`,B.VALUE AS PRNT FROM `TREE_PROFILING_RES` A
			JOIN (SELECT * FROM `TREE_PROFILING_RES` ) B ON A.PARENTID = B.ID
			WHERE A.`VALUE` = '".$ssss."'
			AND A.`FIELD` = 'kabkot'
		";
	 ...
	 $db = $this->clickhouse->db();
		if($param['date'] == $param['dateend']){
			...
			$query2 = "SELECT PROGRAM FROM `PTV_M_SUM_TV_DASH_PROG_DAYS`
  			WHERE `DAYS` = '".$param['date']."' 
			...
			$query2 = "SELECT PROGRAM FROM `PTV_M_SUM_TV_DASH_PROG_DAYS`
  			WHERE `DAYS` BETWEEN '".$param['date']."' AND '".$param['dateend']."' 
  			AND CHANNEL = '".$param['channel']."'
			GROUP BY PROGRAM
			ORDER BY PROGRAM";       
	...
	function list_program_bb($param){  	
	...
	function list_program_ss($param){  	
	
	$date_ff = $param['start_date'][2]."-".$param['start_date'][1]."-".$param['start_date'][0];
	$name_tbs_new = strtoupper(date_format(date_create($date_ff),"Ym"));  
	
	$db = $this->clickhouse->db();

	 if($param['data'] == 'audience'){
			...
			SELECT A.*,CNT FROM (
				SELECT GENRE_PROGRAM,PROGRAM,CHANNEL,formatDateTime(`BEGIN_PROGRAM`,'%H:%M:%S') AS JAM_MULAI,formatDateTime(`END_PROGRAM`,'%H:%M:%S') AS JAM_AKHIR ,
				COUNT(DISTINCT(CARDNO)) AS AUDIENCE
				FROM `CDR_EPG_ALL_SESSION_HARIAN_".$name_tbs_new."_N`
				WHERE CARDNO IS NOT NULL 
				".$param['channel']." ".$param['w_program']."
				AND `DATE` BETWEEN '".$param['start_date'][2]."-".$param['start_date'][1]."-".$param['start_date'][0]."' AND '".$param['end_date'][2]."-".$param['end_date'][1]."-".$param['end_date'][0]."'
				AND (formatDateTime(`BEGIN_PROGRAM`,'%H:%M:%S') BETWEEN '".$param['time_segment'][0]."' AND '".$param['time_segment'][1]."'
				OR formatDateTime(`END_PROGRAM`,'%H:%M:%S') BETWEEN '".$param['time_segment'][0]."' AND '".$param['time_segment'][1]."' )
				GROUP BY GENRE_PROGRAM,CHANNEL,PROGRAM,formatDateTime(`BEGIN_PROGRAM`,'%H:%M:%S'),formatDateTime(`END_PROGRAM`,'%H:%M:%S')

			) A JOIN (

				SELECT CHANNEL,PROGRAM,COUNT(PROGRAM) CNT,JAM_MULAI,JAM_AKHIR FROM (
					SELECT CHANNEL ,PROGRAM,formatDateTime(`BEGIN_PROGRAM`,'%H:%M:%S') AS JAM_MULAI,formatDateTime(`END_PROGRAM`,'%H:%M:%S') AS JAM_AKHIR FROM `CDR_EPG_ALL_SESSION_HARIAN_".$name_tbs_new."_N`
					WHERE CARDNO IS NOT NULL ".$param['channel']." ".$param['w_program']."
					AND `DATE` BETWEEN '".$param['start_date'][2]."-".$param['start_date'][1]."-".$param['start_date'][0]."' AND '".$param['end_date'][2]."-".$param['end_date'][1]."-".$param['end_date'][0]."'
					AND (formatDateTime(`BEGIN_PROGRAM`,'%H:%M:%S') BETWEEN '".$param['time_segment'][0]."' AND '".$param['time_segment'][1]."'
					OR formatDateTime(`END_PROGRAM`,'%H:%M:%S') BETWEEN '".$param['time_segment'][0]."' AND '".$param['time_segment'][1]."' )
					GROUP BY CHANNEL,PROGRAM,`DATE`,JAM_MULAI,JAM_AKHIR
				) A GROUP BY CHANNEL,PROGRAM,JAM_MULAI,JAM_AKHIR
			) B ON A.PROGRAM = B.PROGRAM AND A.JAM_MULAI = B.JAM_MULAI AND A.JAM_AKHIR = B.JAM_AKHIR
			...
			$query2 = "
			SELECT GENRE_PROGRAM,PROGRAM,CHANNEL,JAM_MULAI,JAM_AKHIR,COUNT(PROGRAM) AS CNT
			, SUM(AUDIENCE) AS AUDIENCE, SUM(TOTAL_VIEWS) AS TOTAL_VIEWS, SUM(DURATION) AS DURATION FROM `M_SUMMARY_MEDIA_PLAN_D_PTV_NEW_DTV`
			WHERE PROFILE_ID = 0 ".$param['channel']." ".$param['w_program']."
			AND `DATE` BETWEEN '".$param['start_date'][2]."-".$param['start_date'][1]."-".$param['start_date'][0]."' AND '".$param['end_date'][2]."-".$param['end_date'][1]."-".$param['end_date'][0]."'
			AND (JAM_MULAI BETWEEN '".$param['time_segment'][0]."' AND '".$param['time_segment'][1]."'
			OR JAM_AKHIR BETWEEN '".$param['time_segment'][0]."' AND '".$param['time_segment'][1]."'  )
			GROUP BY CHANNEL,PROGRAM,JAM_MULAI,JAM_AKHIR,GENRE_PROGRAM
	 ...
	 public function get_genre($param){   
		...
		$sql = "	SELECT GENRE_PROGRAM FROM `CDR_EPG_ALL_SESSION_HARIAN_".$name_tbs_new."_N`
						WHERE PROGRAM = '".$param['program']."'
						GROUP BY `GENRE_PROGRAM`";
