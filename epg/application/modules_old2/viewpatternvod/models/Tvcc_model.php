...
ORDER BY `DATE`,M1
	...
	public function list_chart_vod($params = array()) {	
		...
		IF($params['data'] == 'audience'){
				...
				$sql = "
				SELECT MINUTES,
				count(DISTINCT(DURASI_USEETV_SESS)) AS `VIEWERS`, 
				0 AS ID_PROFILE FROM ( 
				SELECT *,DATE_FORMAT(SPLIT_MINUTES,'%H:%i:%s') AS MINUTES FROM INRATE_CDR_VOD_SPLIT A WHERE
			...
			$sql = "
				...
				SELECT MINUTES,
					SUM(GAP) AS `VIEWERS`, 
				0 AS ID_PROFILE FROM ( 
				SELECT *,DATE_FORMAT(SPLIT_MINUTES,'%H:%i:%s') AS MINUTES,
