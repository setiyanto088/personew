...
$(document).ready(function() {
  ...
  $select.each(function() {
    ...
    $(this).next('.urate-select-dropdown').append($hiddenElement).append("<button class='urate-custom-button' id='custom_"+this.id+"'>"+$(this).attr("title")+"</button>").append("<ul class='urate-custom-menu'></ul>");
    ...
    for (var i=0; i<$options.length; i++) {
      if (!($target[i]==null)) {
        $(this).next('.urate-select-dropdown').find("ul.urate-custom-menu").append("<li class='modal-link'><a href='javascript:;' data-toggle='modal' data-target='"+$target[i]+"' data-real='"+$options[i].value+"' data-id='"+this.id+"'>"+$options[i].text+"</a></li>");
      } else {
        $(this).next('.urate-select-dropdown').find("ul.urate-custom-menu").append("<li data-for='"+this.id+"'><a href='javascript:;' data-real='"+$options[i].value+"' data-id='"+this.id+"'>"+$options[i].text+"</a></li>");
      }
    }
  ...
  //$('.multiple-menu .urate-custom-menu > li').click(function() {
  //  $(this).closest('.default').removeClass('active');
  //});
  ...
  $('.urate-select-layout-menu').each(function() {
    ...
    $(this).next('.urate-select-dropdown-layout').append($hiddenElement).append("<button class='urate-custom-button' id='custom_"+this.id+"'>"+$(this).attr("title")+"</button>").append("<ul class='urate-custom-menu'></ul>");

    for (var i=0; i<$options.length; i++) {
      $(this).next('.urate-select-dropdown-layout').find("ul.urate-custom-menu").append("<li class='selected'><a href='javascript:;' data-real='"+$options[i].value+"' data-id='"+this.id+"'>"+$options[i].text+"</a></li>");
    }
