...
class Home_model extends CI_Model {
	...
	public function get_total_interview(){
		...
		$sql = "
		select sum(urban)+sum(rural) total_call from (
	...
	SELECT KOTA_KABUPATEN_DAGRI, IF(SES_SEGMENT = 'RURAL', 1, 0) RURAL, IF(SES_SEGMENT = 'URBAN', 1, 0) URBAN FROM ( SELECT * FROM ( SELECT RANK() OVER (PARTITION BY no_entri ORDER BY start_time) rr ,* FROM t_kuisioner ) s WHERE rr = 1 ) a 
	JOIN RESPONDENT_SURVEY_CLEAR b ON a.no_entri = b.NOTEL_INET
	WHERE (status_survey = 1 or curr_page = 9)
	) F
	GROUP BY KOTA_KABUPATEN_DAGRI
