...
class History_supervisor_model extends CI_Model {
	...
	public function get_history($data){		
		$sql = 
		'SELECT 
			d.nama as surveyor, c.status_survey, a.*,
			IF(respond = 7,IF(c.`status_survey` = 1,8,7 ),respond) respond_res,DATE_FORMAT(time_call,"%d %M %Y") AS dd,DATEDIFF(date_survey, NOW()) sa, 
			`KOTA_KABUPATEN_DAGRI` `KOTA_X`,b.`NO_HP`,`ALAMAT_NOSS` `ALAMAT`,b.`NAMA` NAMA_PELANGGAN, b.KECAMATAN_DAGRI, b.KELURAHAN_DESA_DAGRI 
	...
	public function get_history_filter($data){
		...
		if($data['surveyor'] <> ''){
			$where_kotas = "AND id_user = ".$data['surveyor']."";
		} else{
			$where_kotas = "";
		}	
		...
		(
			SELECT 
				d.nama as surveyor, c.status_survey, a.cardno, a.date_survey, 
				a.day_survey, a.hours_survey, a.date_hours_survey, a.time_call,a.id_outbound,
				IF(respond = 7, IF(c.`status_survey` = 1,8,7 ), respond) respond_res,
				DATE_FORMAT(time_call,"%d %M %Y") AS dd,
				DATEDIFF(date_survey, NOW()) sa, a.id_user,
				`KOTA_KABUPATEN_DAGRI` `KOTA_X`,
				b.`NO_HP`, `ALAMAT_NOSS` `ALAMAT`,
