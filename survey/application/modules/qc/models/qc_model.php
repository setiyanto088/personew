...
class Qc_model extends CI_Model {
	...
	public function get_history_summ($data){
		...
		// if(unvalidate_sum is null,0,unvalidate_sum) unvalidate_sum
		 // FROM t_user_hierarcy a LEFT JOIN (
			// SELECT supervisor,KOTA_X,id, SUM(valid_sum) AS valid_sum,SUM(invalid_sum) AS invalid_sum,SUM(unvalidate_sum) AS unvalidate_sum
		...
		SELECT  supervisor,GROUP_CONCAT(KOTA_X) AS KOTA_X,
		SUM(valid_sum) AS valid_sum,
		SUM(invalid_sum) AS invalid_sum,
		SUM(unvalidate_sum) AS unvalidate_sum,
		SUM(valid_supervisor_sum) AS valid_supervisor_sum  FROM (
		...
		SUM(invalid_sum) AS invalid_sum,
		SUM(unvalidate_sum) AS unvalidate_sum,
		SUM(valid_supervisor_sum) AS valid_supervisor_sum 
		FROM (
						SELECT supervisor,KOTA_X,id,
						IF(valid = 1,1,0) AS valid_sum,
						IF(valid = 3,1,0) AS valid_supervisor_sum,
	...
	public function get_history_filter($data){
		...
		if($data['list_location'] <> ''){
			...
			if(strtoupper($data['list_location']) == 'PEMANTANG SIANTAR'){
				$data['list_location'] = 'PEMATANG SIANTAR';
			}
			
			$where_kota = " AND KOTA_X LIKE '%".strtoupper($data['list_location'])."'";
