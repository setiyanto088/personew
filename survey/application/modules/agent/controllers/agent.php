...
class Agent extends MX_Controller {
	...
	public function index()
	{
			...
			//$this->template->load('maintemplate', 'agent/views/detail', $data);
			$this->template->load('maintemplate', 'agent/views/home', $data);
		...
		// }else {
			...
			if (!isset($upload_error)) {
			   ...
			   // echo $password_generated; 
				...
				$last_id = $this->agent_model->get_last_user();
				$last_id_group = $this->agent_model->get_last_group();
				...
				$result = $this->agent_model->add_user($data);
				...
				$result2 = $this->agent_model->add_user_group($data2);
	...
	public function edit_user() {
				...
				$result = $this->agent_model->edit_user($data);
				...
				if( $this->input->post('password', TRUE) == "00000000" ){
					$result2 = $this->agent_model->edit_group_wop($data2);
				}else{
					$result2 = $this->agent_model->edit_group_wp($data2);
				}
	...
	function lists() {
		 ...
		 // print_r($params);die;
			...
			$list = $this->agent_model->list_po($params); 
	...
	public function detail(){
			...
			$result = $this->agent_model->get_detail($data['id']);
