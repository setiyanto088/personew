...
class Kord_supervisor extends MX_Controller {

    public function __construct()
	{
		parent::__construct();
		$this->load->model('kord_supervisor_model');		
	...
	public function index()
	{
			...
			$data['user'] = $this->kord_supervisor_model->get_user($session_val['user_id']);
			$data['location'] =$this->kord_supervisor_model->get_location($session_val['user_id']);
			...
			foreach($data['location'] as $location){
				...
				$kecamatan =$this->kord_supervisor_model->get_kecamatan($location);
			...
			//$this->template->load('maintemplate', 'kord_supervisor/views/detail', $data);
			$this->template->load('maintemplate', 'kord_supervisor/views/home', $data);
			...
			//if (!isset($upload_error)) {
			   ...
			   // echo $password_generated; 
				$password_hash = password_hash($this->Anti_sql_injection($this->input->post('password_new', TRUE)), PASSWORD_BCRYPT);
				 ...
				 // }
				 // die;
				 $u_id = $this->kord_supervisor_model->get_username($username);
				 
				if($u_id[0]['cnt_user'] == 0){
				 
				$message = "";

				$last_id = $this->kord_supervisor_model->get_last_user();
				$last_id_group = $this->kord_supervisor_model->get_last_group();
				...
				$result = $this->kord_supervisor_model->add_user($data);
				//print_r($result);die;
				...
				$result2 = $this->kord_supervisor_model->add_user_group($data2);
				...
				$this->kord_supervisor_model->mapping_kecamatan($array_batch);
				$this->kord_supervisor_model->mapping_user($data2);
				
				
				if ($result > 0) {
					
					$location =$this->kord_supervisor_model->get_location($user_id);
					$user = $this->kord_supervisor_model->get_user($user_id);
	...
	public function edit_user() {
				...
				$result = $this->kord_supervisor_model->edit_user($data);
				...
				if( $this->input->post('password', TRUE) == "00000000" ){
					$result2 = $this->kord_supervisor_model->edit_group_wop($data2);
				}else{
					$result2 = $this->kord_supervisor_model->edit_group_wp($data2);
				}
	...
	function lists() {
		 ...
		 // print_r($params);die;
			...
			$list = $this->kord_supervisor_model->list_po($params); 
	...
	public function detail(){
			...
			$result = $this->kord_supervisor_model->get_detail($data['id']);
