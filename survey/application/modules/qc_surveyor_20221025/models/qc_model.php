...
class Qc_model extends CI_Model {
	...
	public function get_history_summ($data){
		...
		$sql = '
			...
			SELECT c .nama AS supervisor,GROUP_CONCAT(d.`KECAMATAN`) AS KOTA_X,
				IF(valid_sum IS NULL,0,valid_sum) valid_sum,
				IF(invalid_sum IS NULL,0,invalid_sum) invalid_sum,
				if(valid_supervisor_sum is null,0,valid_supervisor_sum) valid_supervisor_sum,
				IF(unvalidate_sum IS NULL,0,unvalidate_sum) unvalidate_sum
				 FROM t_user_hierarcy a LEFT JOIN (
					SELECT supervisor,surveyor,KOTA_X,id, SUM(valid_sum) AS valid_sum,SUM(invalid_sum) AS invalid_sum,SUM(unvalidate_sum) AS unvalidate_sum
					,SUM(valid_supervisor_sum) AS valid_supervisor_sum  FROM (
						SELECT supervisor,surveyor,KOTA_X,id,
						IF(valid = 1,1,0) AS valid_sum,
						IF(valid = 3,1,0) AS valid_supervisor_sum,
						IF(valid = 2,1,0) AS invalid_sum,
						IF(valid = 0 OR valid = "" OR valid IS NULL,1,0) AS unvalidate_sum FROM (
						
						SELECT g.id_qc,g.`valid`,GROUP_CONCAT(f.nama) AS supervisor,d.nama AS surveyor,h.parent,d.id,
						IF(id_qc IS NULL,c.status_survey,IF(valid_stat = 1,3,4)) status_survey , a.*,
						IF(respond = 7,IF(c.`status_survey` = 1,8,7 ),IF(respond IS NULL,1,respond)) respond_res,DATE_FORMAT(time_call,"%d %M %Y") AS dd,
						DATEDIFF(date_survey, NOW()) sa, `KOTA_KABUPATEN_DAGRI` `KOTA_X`,b.`NO_HP`,`ALAMAT_NOSS` `ALAMAT`,b.`NAMA` NAMA_PELANGGAN,
						 b.KECAMATAN_DAGRI, b.KELURAHAN_DESA_DAGRI FROM `t_outbond_call` a
					...
					) a GROUP BY surveyor,KOTA_X
					ORDER BY surveyor,KOTA_X
					) b ON a.`id_user` = b.id
					LEFT JOIN u_user c ON a.id_user = c.id
					LEFT JOIN `mapping_kecamatan` d ON b.id = d.`ID_USER`
					WHERE a.`parent` = '.$data.'
					GROUP BY a.`id_user`
	...
	public function get_data_qc($user_id){
		...
		$sql 	= '
					...
					SELECT COUNT(id_qc) AS cnt,SUM(valids) valid, SUM(tidak_valid) AS tidak_valid, SUM(belum_validasi) AS belum_validasi FROM (
					SELECT a.*,k.id_qc,
						IF(valid IN (1,3),1,0) AS valids,
					...
					WHERE status_survey = 1 AND h.parent = '.$user_id.'
					) a
