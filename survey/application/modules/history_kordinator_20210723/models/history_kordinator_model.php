...
class History_kordinator_model extends CI_Model {
	...
	public function get_supervisor($data){
		...
		select id_user,nama from t_user_hierarcy a
	...
	public function get_kuisioner($data){
		...
		$sql 	= '
					SELECT GROUP_CONCAT(f.nama) AS supervisor,d.nama AS surveyor,c.status_survey, a.*,IF(respond = 7,IF(c.`status_survey` = 1,8,7 ),IF(respond IS NULL,1,respond)) respond_res,DATE_FORMAT(time_call,"%d %M %Y") AS dd,DATEDIFF(date_survey, NOW()) sa, `KOTA_KABUPATEN_DAGRI` `KOTA_X`,b.`NO_HP`,`ALAMAT_NOSS` `ALAMAT`,b.`NAMA` NAMA_PELANGGAN, b.KECAMATAN_DAGRI, b.KELURAHAN_DESA_DAGRI FROM `t_outbond_call` a
					...
					WHERE a.id_user IN ( SELECT b.id_user FROM t_user_hierarcy a
