...
class Kordinator_profile extends MX_Controller {

    public function __construct()
	{
		parent::__construct();
		$this->load->model('kordinator_profile_model');		
	...
	public function index()
	{
			...
			$data['user'] = $this->kordinator_profile_model->get_user(); 
			
			//print_r($data['user']);die;
		
			//$this->template->load('maintemplate', 'kordinator_profile/views/detail', $data);
			$this->template->load('maintemplate', 'kordinator_profile/views/home', $data);
		...
		// }else {
			...
			if (!isset($upload_error)) {
			   ...
			   // echo $password_generated; 
				...
				$last_id = $this->kordinator_profile_model->get_last_user();
				$last_id_group = $this->kordinator_profile_model->get_last_group();
				...
				$result = $this->kordinator_profile_model->add_user($data);
				...
				$result2 = $this->kordinator_profile_model->add_user_group($data2);
	...
	public function edit_user() {
				...
				$result = $this->kordinator_profile_model->edit_user($data);
				...
				if( $this->input->post('password', TRUE) == "00000000" ){
					$result2 = $this->kordinator_profile_model->edit_group_wop($data2);
				}else{
					$result2 = $this->kordinator_profile_model->edit_group_wp($data2);
				}
	...
	function lists() {
		 ...
		 // print_r($params);die;
			...
			$list = $this->kordinator_profile_model->list_po($params); 
	...
	public function detail(){
			...
			$result = $this->kordinator_profile_model->get_detail($data['id']);
