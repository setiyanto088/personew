...
class History_kordinator_model extends CI_Model
{
	...
	public function list_po($params = array())
	{
      ...
      'SELECT COUNT(*) AS jumlah FROM t_output a WHERE 1 = 1 
		...
		$query2 =
      'SELECT OP.*, rank() over ( ORDER BY user_id ASC) AS Rangking 
		FROM 
		( 
			SELECT *, val_1+val_2+val_3+val_4+val_5+val_6 AS t_val  
			FROM 
			(
				SELECT *,
	...
	{
		$sql =
      'SELECT *,KOTA_KABUPATEN_DAGRI AS KOTA_X 
		FROM RESPONDENT_SURVEY_CLEAR A 
	...
	public function get_supervisor($data)
	{
		$sql = 'SELECT id_user, nama FROM t_user_hierarcy a JOIN u_user b on a.id_user = b.id WHERE parent = ' . $data . ' ORDER BY nama';
		$query_all = $this->db->query($sql);
		$result = $query_all->result_array();
	...
	{
		$sql =
		'SELECT a.`field_raw`,a.`type_input`,a.`code_question`,a.`sub_question`,b.* 
		FROM `t_question_value` a
	...
	public function get_history($data)
	{
		$sql =
		'SELECT 
			GROUP_CONCAT(f.nama) AS supervisor, d.nama AS surveyor, c.status_survey, a.*,
			IF(respond = 7, IF(c.`status_survey` = 1,8,7 ), IF(respond IS NULL,1,respond)) respond_res,
			DATE_FORMAT(time_call,"%d %M %Y") AS dd, DATEDIFF(date_survey, NOW()) sa, 
			`KOTA_KABUPATEN_DAGRI` `KOTA_X`,
			b.`NO_HP`, `ALAMAT_NOSS` `ALAMAT`, b.`NAMA` NAMA_PELANGGAN,
			b.KECAMATAN_DAGRI, b.KELURAHAN_DESA_DAGRI 
		...
		( 
			SELECT b.id_user 
			FROM t_user_hierarcy a
			JOIN t_user_hierarcy b ON a.id_user = b.parent 
			WHERE a.parent = ? 
		) 
		GROUP BY a.cardno
	...
	public function get_history_filter($data)
	{
		...
		if ($data['texts'] <> '') {
			...
			"AND 
			( 
				NAMA_PELANGGAN LIKE '%" . $data['texts'] . "%' OR cardno LIKE '%" . $data['texts'] . "%' 
				OR surveyor LIKE '%" . $data['texts'] . "%' 
				OR supervisor LIKE '%" . $data['texts'] . "%' OR ALAMAT LIKE '%" . $data['texts'] . "%' 
			)";
		} else {
			$where_haris = "";
		}
		...
		(
			SELECT 
				...
				IF(respond = 7, IF(c.`status_survey` = 1,8,7 ), IF(respond IS NULL,1,respond)) respond_res,
				DATE_FORMAT(time_call,"%d %M %Y") AS dd, DATEDIFF(date_survey, NOW()) sa, 
				`KOTA_KABUPATEN_DAGRI` `KOTA_X`,b.`NO_HP`,`ALAMAT_NOSS` `ALAMAT`,
				b.`NAMA` NAMA_PELANGGAN, b.KECAMATAN_DAGRI, b.KELURAHAN_DESA_DAGRI 
			FROM `t_outbond_call` a
			LEFT JOIN `RESPONDENT_SURVEY_CLEAR` b ON a.cardno = b.`NOTEL_INET`
			LEFT JOIN `t_kuisioner` c ON a.id_outbound = c.id_outbound
			LEFT JOIN `u_user` d ON a.id_user = d.id
	...
	public function get_cardno($user_id)
	{
		...
		$sql 	= 
      'SELECT b.*,b.NOTEL_INET as CARDNO 
      FROM `t_outbond_call` a 
      JOIN RESPONDENT_SURVEY_CLEAR b ON a.cardno = b.NOTEL_INET
		WHERE a.id_user = "' . $user_id . '" AND a.respond = 7
	...
	{
		$sql 	= 
      "SELECT `FIELD`, SUBSTRING_INDEX(`FIELD`,'_',-1)AS MERK_CAR 
      FROM `TREE_PROFILING_RES`
	...
	{
		$sql 	= 
      "SELECT `FIELD`, SUBSTRING_INDEX(`FIELD`,'_',-1) AS MERK_MB 
      FROM `TREE_PROFILING_RES`
	...
	{
		$sql 	= 
      "SELECT *, KOTA_KABUPATEN_DAGRI AS KOTA_X, NAMA AS NAMA_PELANGGAN, NOTEL_INET AS CARDNO, ALAMAT_NOSS AS ALAMAT 
      FROM RESPONDENT_SURVEY_CLEAR A
	...
	{
		$sql 	= "SELECT * FROM t_outbond_call WHERE CARDNO = '" . $data['id'] . "' ";
		$query_all = $this->db->query($sql);
		$result = $query_all->result_array();
	...
	{
		$sql 	= "SELECT * FROM t_question_value WHERE `id_category` = " . $data . " ORDER BY seq ASC";
		$query_all = $this->db->query($sql);
		$result = $query_all->result_array();
	...
	{
		$sql 	= "SELECT * FROM t_question_value WHERE `id_category` = " . $data . " and mandatory = 1 ORDER BY seq ASC ";
		$query_all = $this->db->query($sql);
		$result = $query_all->result_array();
	...
	{
		$sql = 
      "DELETE FROM t_form_value WHERE id_question IN 
      ( 
         SELECT id_question 
         FROM t_question_value 
         WHERE id_category = " . $cat . " 
      ) 
      AND id_kuisioner = " . $id_qus . " ";
	...
	public function delete_outbound($id_qus)
	{
      ...
      "INSERT INTO t_outbond_call_history
		...
		$sql 	= "DELETE FROM t_outbond_call WHERE id_outbound = " . $id_qus['id_outbound'] . " ";
		$query_all = $this->db->query($sql);
