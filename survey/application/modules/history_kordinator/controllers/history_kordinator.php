...
class History_kordinator extends MX_Controller {

    public function __construct(){
		parent::__construct();
		$this->load->model('history_kordinator_model');		
	...
	public function insert_survey(){
		...
		$add_prin_result = $this->history_kordinator_model->edit_quesioner($data_respondent);
		$array_form_part = [[1,2],[3],[4],[5],[6],[7],[8]];
		$array_part = $array_form_part[$form];
		
		foreach($array_part as $part){
			$this->history_kordinator_model->delete_curr_answ($part,$data_respondent['id_kuisioner']);
			$r1 = $this->history_kordinator_model->get_field($part);
			foreach($r1 as $r1s){
				$data_r1 = array(
				...
				$this->history_kordinator_model->add_quesioner_part($data_r1);
			}
		}
	}
	...
	public function insert_new_item(){
		...
		$add_prin_result = $this->history_kordinator_model->add_new_item($data_respondent);		
	}	
	...
	public function insert_header_survey(){
		...
		$sss = $this->history_kordinator_model->check_inbound($data_respondent);	
		$this->history_kordinator_model->edit_stat($data_respondent);
		
		if($sss[0]['CNT'] == 0){
			$add_prin_result = $this->history_kordinator_model->add_header_survey($data_respondent);	
		}		
	}
	...
	public function edit_schedule(){
		...
		$this->history_kordinator_model->edit_schedule($param);	
	...
	public function delete_outbound(){
		...
		$this->history_kordinator_model->delete_outbound($datas);	
		// $this->log_activity->insert_activity('insert', 'Insert User');
	...
	public function filter_jadwal(){
		...
		$get_history = $this->history_kordinator_model->get_history_filter($data); 
		$htmls = '';
		...
		'<table id="table_resp_ss" class="table table-striped" style="">
			<thead>
				<tr style="color:#F1646E !important;">
					<td><p class="" ><b>Responden</b></p></td>
					<td><p class="" ><b>Call</b></p></td>
					<td><p class="" ><b>Interview</b></p></td>
					<td><p class="" ><b>Surveyor</b></p></td>
					<td><p class="" ><b>Supervisor</b></p></td>
					<td><p class="" ><b>Status</b></p></td>
				</tr>
			</thead>
			<tbody>';
		...
		foreach($get_history as $users){
			...
			$htmls .= '		<td> '.$users['surveyor'].' </td>';
			$htmls .= '		<td> '.$users['supervisor'].' </td>';
			$htmls .= '		<td> '.$data['respond'][$users['respond_res']].' </td> ';
			$htmls .= '	</tr>';
		}
	...
	public function export() {
		...
		$objPHPExcel
	   		->setActiveSheetIndex(0)
				->setCellValue('A1', 'Nomor Pelanggan')
		...
		$i = 2;
		foreach($get_history as $lists){
			
			$objPHPExcel->setActiveSheetIndex(0)
				->setCellValue('A'.$i, "'".$lists['cardno'])
				->setCellValue('B'.$i, $lists['NAMA_PELANGGAN'])
				->setCellValue('C'.$i, $lists['ALAMAT'])
				->setCellValue('D'.$i, $lists['NO_HP']);
			$i++;
		}
		...
		$objWriter->save('/var/www/html/tmp_doc/report_respondent.xls');	
		echo "excel";
	}
	...
	public function get_respondent(){
		...
		$result  = $this->history_kordinator_model->get_respondent($param);
		$resulte = $this->history_kordinator_model->get_outbound2($param);
	...
	public function new_survey(){
		...
		$result_merk = $this->history_kordinator_model->get_merk();
		$data['data_cardno'] = $this->history_kordinator_model->get_cardno($session_val['user_id']);
		$data['data_car'] 	 = $this->history_kordinator_model->get_merk_car();
		$data['data_mb'] 	 = $this->history_kordinator_model->get_merk_mb();
		$data['outbound'] 	 = $this->history_kordinator_model->get_outbound($id_outbound);
		$data['kuisioner'] 	 = $this->history_kordinator_model->get_kuisioner($id_outbound);
		...
		$get_kuss = $this->history_kordinator_model->get_kuss($id_outbound);
		...
		$data['field_r1'] = $this->history_kordinator_model->get_field(1);
		$data['field_r2'] = $this->history_kordinator_model->get_field(2);
		$data['field_r3'] = $this->history_kordinator_model->get_field(3);
		$data['field_r4'] = $this->history_kordinator_model->get_field(4);
		$data['field_r5'] = $this->history_kordinator_model->get_field(5);
		$data['field_r6'] = $this->history_kordinator_model->get_field(6);
		$data['field_r7'] = $this->history_kordinator_model->get_field(7);
		$data['field_r8'] = $this->history_kordinator_model->get_field(8);
		
		$data['field_r1v'] = $this->history_kordinator_model->get_field_val(1);
		$data['field_r2v'] = $this->history_kordinator_model->get_field_val(2);
		$data['field_r3v'] = $this->history_kordinator_model->get_field_val(3);
		$data['field_r4v'] = $this->history_kordinator_model->get_field_val(4);
		$data['field_r5v'] = $this->history_kordinator_model->get_field_val(5);
		$data['field_r6v'] = $this->history_kordinator_model->get_field_val(6);
		$data['field_r7v'] = $this->history_kordinator_model->get_field_val(7);
		$data['field_r8v'] = $this->history_kordinator_model->get_field_val(8);
			
		//if($data['kuisioner'][0]['status_survey'] == 2){
			$data['data_r1'] = $this->history_kordinator_model->get_data(1,$data['kuisioner'][0]['id_kuisioner']);
			$data['data_r2'] = $this->history_kordinator_model->get_data(2,$data['kuisioner'][0]['id_kuisioner']);
			$data['data_r3'] = $this->history_kordinator_model->get_data(3,$data['kuisioner'][0]['id_kuisioner']);
			$data['data_r4'] = $this->history_kordinator_model->get_data(4,$data['kuisioner'][0]['id_kuisioner']);
			$data['data_r5'] = $this->history_kordinator_model->get_data(5,$data['kuisioner'][0]['id_kuisioner']);
			$data['data_r6'] = $this->history_kordinator_model->get_data(6,$data['kuisioner'][0]['id_kuisioner']);
			$data['data_r7'] = $this->history_kordinator_model->get_data(7,$data['kuisioner'][0]['id_kuisioner']);
			$data['data_r8'] = $this->history_kordinator_model->get_data(8,$data['kuisioner'][0]['id_kuisioner']);
			
			$this->template->load('maintemplate', 'history_kordinator/views/survey_new_resume', $data);
		// }else{
			// $this->template->load('maintemplate', 'history_kordinator/views/survey_new_resume', $data);
		// }
		//$this->template->load('maintemplate', 'history_kordinator/views/detail', $data);
		//$this->template->load('maintemplate', 'history_kordinator/views/survey_new_resume', $data);
	}
	...
	public function index(){
		...
		$data['session'] 	 = $session_val;
		$data['user'] 		 = $this->history_kordinator_model->get_user(); 
		$data['get_history'] = $this->history_kordinator_model->get_history($user_id); 
		$data['kecamatan']   = $this->history_kordinator_model->get_supervisor($user_id);
		...
		//$this->template->load('maintemplate', 'history_kordinator/views/detail', $data);
		$this->template->load('maintemplate', 'history_kordinator/views/home', $data);
	}
