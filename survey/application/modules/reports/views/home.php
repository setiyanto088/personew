 ...
 .table-responsive .responsive{
    max-height:300px;
      ...
      <div class="main-panel">
		...
		<th rowspan=2 style="border: 1px solid black;text-align: center;">Wilayah</th>
                <th rowspan=2 style="border: 1px solid black;text-align: center;">Kota</th>
	...
	if( ($data_survey[$val[0]]['RURAL']/$val[2])*100 >= 100 ){
		$data_rural = $val[2];
	}else{
		$data_rural = $data_survey[$val[0]]['RURAL'];
	}

	if( ($data_survey[$val[0]]['URBAN']/$val[1])*100 >= 100 ){
		$data_urban = $val[1];
	}else{
		$data_urban = $data_survey[$val[0]]['URBAN'];
	}
	...
	<?php } ?>
		...
		<td style="text-align: right;<?php if($total_pers < 31){ echo " color:red;"; }elseif($total_pers > 30 && $total_pers < 79 ){ echo " color:red;"; }elseif($total_pers > 80 && $total_pers < 100 ){ echo " color:#ffd966;"; }else{ echo " color:green;"; } ?>"><strong><h6><?php echo number_format((($data_survey[$val[0]]['RURAL']+$data_survey[$val[0]]['URBAN'])/($val[2]+$val[1]))*100,1,',','.'); ?> %</strong></h6></td>
	</tr>
...
<?php $i++; $total_urban += $data_survey[$val[0]]['URBAN']; $total_rural += $data_survey[$val[0]]['RURAL']; } ?>
		...
		<th style="text-align: right;"><?php echo number_format(($total_urban/3141)*100,1,',','.'); ?> %</th>
                <th>1.859</th>
		<th style="text-align: right;"><?php echo number_format($total_rural,0,',','.'); ?></th>
		<th style="text-align: right;"><?php echo number_format(($total_rural/1859)*100,1,',','.'); ?> %</th>
		<th>5.000</th>
...
function print_excel(){
	...
	$.ajax({
			...
			success: function(data){
				...
				download_file('https://inrate.id/tmp_doc/reports_excel_summary.xls','reports_excel_summary.xls');
