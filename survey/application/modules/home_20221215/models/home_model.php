...
class Home_model extends CI_Model {
	...
	public function get_total_interview(){
		...
		$sql = "
		SELECT 	
	SUM(valid_sum) valid_sum,
	SUM(invalid_sum) invalid_sum,
	SUM(valid_supervisor_sum) valid_supervisor_sum,
	SUM(unvalidate_sum) unvalidate_sum,
	SUM(valid_sum)+SUM(invalid_sum)+SUM(valid_supervisor_sum)+SUM(unvalidate_sum) AS total_call FROM (
		SELECT a.*,
		IF(valid = 1,1,0) AS valid_sum,
		IF(valid = 2,1,0) AS invalid_sum,
		IF(valid = 3,1,0) AS valid_supervisor_sum,
		IF(valid = 0 OR valid = '' OR valid IS NULL,1,0) AS unvalidate_sum
		FROM `t_kuisioner` a
		JOIN `t_outbond_call` c ON a.`id_outbound` = c.`id_outbound`
		JOIN RESPONDENT_SURVEY_CLEAR d ON c.cardno = d.NOTEL_INET
		LEFT JOIN `t_qc` b ON a.`id_outbound` = b.`id_outbound`
		WHERE `status_survey` = 1
	) a
		";
	...
	public function table_data(){
		...
		SELECT a.*,
			...
			FROM (
				SELECT b.id,b.nama,COUNT(distinct(a.cardno)) total_call, RESP AS all_resp FROM t_outbond_call a 
			JOIN u_user b ON a.id_user = b.id
			LEFT JOIN (
				select id,sum(RESP) resp FROM (
					SELECT b.id,c.kecamatan, RESP FROM u_user b 
					LEFT JOIN `mapping_kecamatan` c ON c.`ID_USER` = b.id
					LEFT JOIN `RES_LOCATION` d ON c.kecamatan = d.`KECAMATAN`
					WHERE c.kecamatan is not null
					GROUP BY b.id,c.kecamatan
				) a group by id
			) d on b.id = d.id
			GROUP BY a.id_user
		) a left join (
