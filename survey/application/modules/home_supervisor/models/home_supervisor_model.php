...
class Home_supervisor_model extends CI_Model {
	...
	public function get_total_interview($user_id){
		...
		$sql = '
		SELECT count(*) total_call FROM ( SELECT * FROM ( SELECT RANK() OVER (PARTITION BY no_entri ORDER BY start_time) rr ,b.*,status_survey FROM t_kuisioner a 
				join `t_outbond_call` b on a.`id_outbound` = b.`id_outbound` ) s  ) a 
		join u_user b on a.id_user = b.id
		WHERE a.id_user IN(
						SELECT `id_user` FROM `t_user_hierarcy` a
						WHERE parent = "'.$user_id.'"
	...
	public function list_survey($user_id){
		...
		SELECT A.* FROM (
			...
			SELECT A.KOTA_KABUPATEN_DAGRI, IF(URBAN IS NULL, 0, URBAN) URBAN, IF(RURAL IS NULL,0,RURAL) RURAL,
			IF(RURAL_VALID_SUPERVISOR IS NULL,0,RURAL_VALID_SUPERVISOR) RURAL_VALID_SUPERVISOR, 
			IF(RURAL_INVALID IS NULL,0,RURAL_INVALID) RURAL_INVALID, 
			IF(RURAL_VALID IS NULL,0,RURAL_VALID) RURAL_VALID, 
			IF(URBAN_VALID_SUPERVISOR IS NULL,0,URBAN_VALID_SUPERVISOR) URBAN_VALID_SUPERVISOR, 
			IF(URBAN_VALID IS NULL,0,URBAN_VALID) URBAN_VALID, 
			IF(URBAN_INVALID IS NULL,0,URBAN_INVALID) URBAN_INVALID
			FROM (
				SELECT DISTINCT(KOTA_KABUPATEN_DAGRI) KOTA_KABUPATEN_DAGRI FROM RESPONDENT_SURVEY_CLEAR
				GROUP BY KOTA_KABUPATEN_DAGRI
				) A LEFT JOIN (
				
				SELECT KOTA_KABUPATEN_DAGRI, SUM(URBAN) URBAN, SUM(RURAL) RURAL,
				SUM(RURAL_VALID_SUPERVISOR) RURAL_VALID_SUPERVISOR,
				SUM(RURAL_INVALID) RURAL_INVALID,
				SUM(RURAL_VALID) RURAL_VALID,
				SUM(URBAN_VALID_SUPERVISOR) URBAN_VALID_SUPERVISOR,
				SUM(URBAN_VALID) URBAN_VALID,
				SUM(URBAN_INVALID) URBAN_INVALID FROM (
				SELECT KOTA_KABUPATEN_DAGRI, IF(SES_SEGMENT = 'RURAL', 1, 0) RURAL, IF(SES_SEGMENT = 'URBAN', 1, 0) URBAN,
							IF(SES_SEGMENT = 'RURAL',IF(c.valid_stat = 3,1,0), 0) RURAL_VALID_SUPERVISOR,
							IF(SES_SEGMENT = 'RURAL',IF(c.valid_stat = 1,1,0), 0) RURAL_VALID,
							IF(SES_SEGMENT = 'RURAL',IF(c.valid_stat = 2,1,0), 0) RURAL_INVALID,
							IF(SES_SEGMENT = 'URBAN',IF(c.valid_stat = 3,1,0), 0) URBAN_VALID_SUPERVISOR,
							IF(SES_SEGMENT = 'URBAN',IF(c.valid_stat = 1,1,0), 0) URBAN_VALID,
							IF(SES_SEGMENT = 'URBAN',IF(c.valid_stat = 2,1,0), 0) URBAN_INVALID
							FROM ( SELECT * FROM ( SELECT RANK() OVER (PARTITION BY no_entri ORDER BY start_time) rr ,b.*,status_survey FROM t_kuisioner a 
				join `t_outbond_call` b on a.`id_outbound` = b.`id_outbound` ) s ) a 
				JOIN RESPONDENT_SURVEY_CLEAR b ON a.cardno = b.NOTEL_INET
				LEFT JOIN `t_qc` c ON a.`id_outbound` = c.`id_outbound`
				WHERE a.id_user IN(
						SELECT `id_user` FROM `t_user_hierarcy` a
						WHERE parent = '".$user_id."'
					)
	...
	GROUP BY KOTA_KABUPATEN_DAGRI
		...
		SELECT d.*,IF(location_name = 'Pemantang Siantar','Pematang Siantar',location_name) location_name_n FROM `t_user_location` c
					JOIN t_location d ON c.id_location = d.id_location
					WHERE c.id_user = '".$user_id."'
					GROUP BY d.id_location
	) B ON A.KOTA_KABUPATEN_DAGRI = B.location_name_n
		";
...
GROUP BY KOTA_KABUPATEN_DAGRI
	...
	SELECT a.*,
		...
		LEFT JOIN (
			SELECT b.id,b.nama,COUNT(DISTINCT(cardno)) total_interview, SUM(`RESP`) AS all_resp FROM u_user b
				LEFT JOIN t_outbond_call a  ON a.id_user = b.id
				LEFT JOIN t_kuisioner aa ON a.id_outbound = aa.id_outbound
				LEFT JOIN `mapping_kecamatan` c ON c.`ID_USER` = b.id
				LEFT JOIN `RES_LOCATION` d ON c.kecamatan = d.`KECAMATAN`
				WHERE a.id_user IN(
					SELECT `id_user` FROM `t_user_hierarcy` a
