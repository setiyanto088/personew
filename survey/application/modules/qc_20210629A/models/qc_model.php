...
class Qc_model extends CI_Model {
	...
	public function add_new_qc($data) {
		$sql 	= 'insert into t_qc_history SELECT * FROM t_qc WHERE id_outbound = '.$data['id_outbound'].'';
		$query_all 	=  $this->db->query($sql);
		$this->db->close();
		$this->db->initialize();
		
		
		$sql 	= 'delete from t_qc WHERE id_outbound = '.$data['id_outbound'].'';
		$query_all 	=  $this->db->query($sql);
		$this->db->close();
		$this->db->initialize();
	...
	public function get_data_qc(){
		
		$sql 	= '
		
					select count(id_qc) as cnt,sum(valids) valid, sum(tidak_valid) as tidak_valid,total_survey from (
						select *,if(id_qc =7,1,0) as valids,IF(id_qc <> 7,1,0) AS tidak_valid from `t_qc`
					) a, (select count(*) as total_survey from t_outbond_call a JOIN `t_kuisioner` c ON a.id_outbound = c.id_outbound where status_survey = 1 ) b
	...
	public function get_data_qc_tot(){
		
		$sql 	= '
		
					select count(*) as total_survey from t_outbond_call a JOIN `t_kuisioner` c ON a.id_outbound = c.id_outbound where status_survey = 1 
	...
	public function get_history_filter($data){
		...
		if($data['respond'] <> ''){
			$where_date = "AND status_survey = '".$data['respond']."'";
