...
class Qc extends MX_Controller {
	...
	public function insert_qc()
	{
		...
		$data_respondent = array(
			'info_vali' => $this->Anti_sql_injection($this->input->post('info_vali', TRUE)),
			'demografi_vali' => $this->Anti_sql_injection($this->input->post('demografi_vali', TRUE)),
			'profile_rt_vali' => $this->Anti_sql_injection($this->input->post('profile_rt_vali', TRUE)),
			'internet_data_vali' => $this->Anti_sql_injection($this->input->post('internet_data_vali', TRUE)),
			'menonton_tv_vali' => $this->Anti_sql_injection($this->input->post('menonton_tv_vali', TRUE)),
			'program_tv_vali' => $this->Anti_sql_injection($this->input->post('program_tv_vali', TRUE)),
			'kesan_pemirsa_vali' => $this->Anti_sql_injection($this->input->post('kesan_pemirsa_vali', TRUE)),
			'kegemaran_prilaku_vali' => $this->Anti_sql_injection($this->input->post('kegemaran_prilaku_vali', TRUE)),
			'product_ownership_vali' => $this->Anti_sql_injection($this->input->post('product_ownership_vali', TRUE)),
			'info_note' => $this->Anti_sql_injection($this->input->post('info_note', TRUE)),
			'demografi_note' => $this->Anti_sql_injection($this->input->post('demografi_note', TRUE)),
			'profile_rt_note' => $this->Anti_sql_injection($this->input->post('profile_rt_note', TRUE)),
			'internet_data_note' => $this->Anti_sql_injection($this->input->post('internet_data_note', TRUE)),
			'menonton_tv_note' => $this->Anti_sql_injection($this->input->post('menonton_tv_note', TRUE)),
			'program_tv_note' => $this->Anti_sql_injection($this->input->post('program_tv_note', TRUE)),
			'kesan_pemirsa_note' => $this->Anti_sql_injection($this->input->post('kesan_pemirsa_note', TRUE)),
			'kegemaran_prilaku_note' => $this->Anti_sql_injection($this->input->post('kegemaran_prilaku_note', TRUE)),
			'product_ownership_note' => $this->Anti_sql_injection($this->input->post('product_ownership_note', TRUE)),
			'id_outbound' => $this->Anti_sql_injection($this->input->post('id_outbound', TRUE)),
			'valid' => $this->Anti_sql_injection($this->input->post('valid', TRUE)),
	...
	public function filter_jadwal(){
		...
		$respond_qc = ['<div class="badge badge-success" ><h5>Selesai Survey</h5></div>',
			'<div class="badge badge-success" ><h5>Selesai Survey</h5></div>',
			'<div class="badge badge-success" ><h5>Selesai Survey</h5></div>',
...
'<div class="badge badge-success" ><h5>Selesai Survey</h5></div>',
	...
	//	$data['array_kota'] = explode('|',$kota);
		...
		foreach($get_history as $users){
			...
			if($users['status_survey'] == null ){
				...
				$htmls .= '										<div class="col-md-2" style="align:center;" ><br>'.$respond_qc[$users['status_survey']].'<br><br><button type="button" class="btn btn-info btn-sm" onClick="check_outbound('.$users['id_outbound'].')">Check</button></div>';
				$htmls .= '									</div>';
				$htmls .= '								</div>';
				$htmls .= '							</div>';
	...
	public function index()
	{
			...
			$data['kecamatan'] = $this->qc_model->get_supervisor();
			$data['data_qc'] = $this->qc_model->get_data_qc();
			$data['data_qc_tot'] = $this->qc_model->get_data_qc_tot();
