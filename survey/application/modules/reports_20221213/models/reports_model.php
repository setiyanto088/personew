...
SELECT KOTA_KABUPATEN_DAGRI,c.cardno, IF(SES_SEGMENT = 'RURAL', 1, 0) RURAL, IF(SES_SEGMENT = 'URBAN', 1, 0) URBAN FROM t_kuisioner a 
JOIN t_outbond_call c ON c.id_outbound = a.id_outbound
JOIN RESPONDENT_SURVEY_CLEAR b ON c.cardno = b.NOTEL_INET
join t_qc d on a.id_outbound = d.id_outbound

WHERE status_survey <> 0
and valid = 1
GROUP BY cardno
) F
GROUP BY KOTA_KABUPATEN_DAGRI
	...
	public function table_data(){
		...
		SELECT a.*,
			...
			FROM (
				SELECT b.id,b.nama,COUNT(a.id_outbound) total_call, SUM(`RESP`) AS all_resp FROM t_outbond_call a 
	...
	public function get_chart_interview($data){
		...
		where status_survey <> 0 AND KOTA_KABUPATEN_DAGRI = '".$data."' 
