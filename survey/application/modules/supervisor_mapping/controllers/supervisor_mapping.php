...
class Supervisor_mapping extends MX_Controller {

    public function __construct()
	{
		parent::__construct();
		$this->load->model('supervisor_mapping_model');		
	...
	public function index()
	{
			...
			$data['user'] = $this->supervisor_mapping_model->get_user($session_val['user_id']);
			//$data['location'] =$this->supervisor_mapping_model->get_location($session_val['user_id']);
			$data['get_history'] = $this->supervisor_mapping_model->get_history($user_id); 
			$data['get_chart'] = $this->supervisor_mapping_model->get_chart($user_id); 

			$data['location'] =$this->supervisor_mapping_model->get_locations($session_val['user_id']);
			//$data['kecamatan'] =$this->supervisor_mapping_model->get_kecamatan($data['location'][0]);
			...
			foreach($data['location'] as $location){
				...
				$kecamatan =$this->supervisor_mapping_model->get_kecamatan($location);
			...
			$array_chart = $data['get_chart'][0]['Jan'].",".$data['get_chart'][0]['Feb'].",".$data['get_chart'][0]['Mar'].",".$data['get_chart'][0]['Apr'].",".$data['get_chart'][0]['May'].",".$data['get_chart'][0]['Jun'].",".$data['get_chart'][0]['Jul'].",".$data['get_chart'][0]['Aug'].",".$data['get_chart'][0]['Sep'].",".$data['get_chart'][0]['Oct'].",".$data['get_chart'][0]['Nov'].",".$data['get_chart'][0]['Dec'].",";
			
			// foreach($data['get_chart'] as $ach){
			...
			$data['array_chart'] = $array_chart;
			//print_r($data['location']);die;
		
			//$this->template->load('maintemplate', 'supervisor_mapping/views/detail', $data);
			$this->template->load('maintemplate', 'supervisor_mapping/views/home', $data);
		
	}
	
	public function get_resume(){
		
		$users = strtolower($this->Anti_sql_injection($this->input->post('users', TRUE)));
	
		$data['location'] =$this->supervisor_mapping_model->get_locations($users);
		$data['kecamatan'] =$this->supervisor_mapping_model->get_kecamatan_user($data['location'][0]['location_name'],$users);
		...
		//print_r($data);die;
		/*$get_resume = $this->supervisor_mapping_model->get_resume($users); 
		$data['get_history'] = $this->supervisor_mapping_model->get_history($users); 
		$data['get_chart'] = $this->supervisor_mapping_model->get_chart($users); 
		
		$array_chart[] = intval($data['get_chart'][0]['Jan']);
		$array_chart[] = intval($data['get_chart'][0]['Feb']);
		$array_chart[] = intval($data['get_chart'][0]['Mar']);
		$array_chart[] = intval($data['get_chart'][0]['Apr']);
		$array_chart[] = intval($data['get_chart'][0]['May']);
		$array_chart[] = intval($data['get_chart'][0]['Jun']);
		$array_chart[] = intval($data['get_chart'][0]['Jul']);
		$array_chart[] = intval($data['get_chart'][0]['Aug']);
		$array_chart[] = intval($data['get_chart'][0]['Sep']);
		$array_chart[] = intval($data['get_chart'][0]['Oct']);
		$array_chart[] = intval($data['get_chart'][0]['Nov']);
		$array_chart[] = intval($data['get_chart'][0]['Dec']);
			
		
		$html = '';
		 foreach($data['get_history'] as $users){ 
																
																$array_akses = ['<span style="color:red">Not Active</span>','<span style="color:green">Active</span>'];
															
																$html .= "<tr style='border-bottom: 1px solid grey;'><td width='5%' style='font-size: 12px'><img style='border-radius:10%;background-color:#F1646E;width:20px;height:20px;margin-right:10px' src='".base_url().'assets/survey/Group105.png'."' alt='profile'/></td><td style='font-size: 12px'>".$users['NAMA_PELANGGAN']."</td><td style='font-size: 12px'>".$users['NO_HP']."</td><td style='font-size: 12px'>".$users['dd']."</td style='font-size: 12px'></tr>";
															}
		
		
		$data['get_resume'] = $get_resume[0];
		$data['body_history'] = $html;
		...
		foreach($data['kecamatan'] as $kecamatan ){
			...
			<div class="form-check">
				<label class="form-check-label">
				      <input type="checkbox" name="kecamatan[]" value="'.$kecamatan['KECAMATAN'].'" class="form-check-input" >  
				      '.$kecamatan['KECAMATAN'].'
				</label>
			      </div>
		      </div>';
		...
		//$data['get_mapping'] = $this->supervisor_mapping_model->get_mapping($users); 				

		$result = array('success' => true, 'message' => '','his' => $data['location'] );
	...
	public function insert_mapping(){
		...
		$id_user = $this->Anti_sql_injection($this->input->post('id_user', TRUE));
		...
		$kecamatan_array = explode('|',$kecamatan);
				...
				foreach($kecamatan_array as $kecamatan_arrays){
					...
					$array_batch[$ii]['id_user'] = $id_user;
					$array_batch[$ii]['id_location'] = $kecamatan_arrays;
					
					$ii++;
				}
				
				$this->supervisor_mapping_model->delete_mapping_kecamatan($id_user);

				$this->supervisor_mapping_model->mapping_kecamatan($array_batch);

				$result = array('success' => true, 'message' => 'Berhasil Mapping User', 'html' => '' );
			...
			//if (!isset($upload_error)) {
			   ...
			   // echo $password_generated; 
				$password_hash = password_hash($this->Anti_sql_injection($this->input->post('password_new', TRUE)), PASSWORD_BCRYPT);
				 ...
				 // }
				 // die;
				// $user_cnt = $this->supervisor_mapping_model->get_username($username);
				 
				// print_r($user_cnt);die;
				 
				$message = "Username " . $username . " sudah terdaftar, silahkan pilih usernam yang lain";

				$last_id = $this->supervisor_mapping_model->get_last_user();
				$last_id_group = $this->supervisor_mapping_model->get_last_group();
				...
				$result = $this->supervisor_mapping_model->add_user($data);
				//print_r($result);die;
				...
				$result2 = $this->supervisor_mapping_model->add_user_group($data2);
				
				if ($result > 0) {
					
					$location =$this->supervisor_mapping_model->get_location($idlokasi);
					$user = $this->supervisor_mapping_model->get_user($idlokasi);
	...
	public function edit_user() {
				...
				$result = $this->supervisor_mapping_model->edit_user($data);
				...
				if( $this->input->post('password', TRUE) == "00000000" ){
					$result2 = $this->supervisor_mapping_model->edit_group_wop($data2);
				}else{
					$result2 = $this->supervisor_mapping_model->edit_group_wp($data2);
				}
	...
	function lists() {
		 ...
		 // print_r($params);die;
			...
			$list = $this->supervisor_mapping_model->list_po($params); 
	...
	public function detail(){
			...
			$result = $this->supervisor_mapping_model->get_detail($data['id']);
