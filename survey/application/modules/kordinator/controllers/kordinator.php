...
class Kordinator extends MX_Controller {
	...
	public function index()
	{
			...
			$data['user'] = $this->kordinator_model->get_user($session_val['idlokasi']);
			$data['location'] =$this->kordinator_model->get_location($session_val['user_id']);
			...
			foreach($data['location'] as $location){
				...
				$kecamatan =$this->kordinator_model->get_kecamatan($location);
			...
			//$this->template->load('maintemplate', 'kordinator/views/detail', $data);
			$this->template->load('maintemplate', 'kordinator/views/home', $data);
		...
		// }else {
			...
			//if (!isset($upload_error)) {
			   ...
			   // echo $password_generated; 
				...
				$last_id = $this->kordinator_model->get_last_user();
				$last_id_group = $this->kordinator_model->get_last_group();
				...
				$result = $this->kordinator_model->add_user($data);
				//print_r($result);die;
				...
				$result2 = $this->kordinator_model->add_user_group($data2);
				...
				$this->kordinator_model->mapping_kecamatan($array_batch);
				
				
				if ($result > 0) {
					
					$location =$this->kordinator_model->get_location($user_id);
					$user = $this->kordinator_model->get_user($idlokasi);
	...
	public function edit_user() {
				...
				$result = $this->kordinator_model->edit_user($data);
				...
				if( $this->input->post('password', TRUE) == "00000000" ){
					$result2 = $this->kordinator_model->edit_group_wop($data2);
				}else{
					$result2 = $this->kordinator_model->edit_group_wp($data2);
				}
	...
	function lists() {
		 ...
		 // print_r($params);die;
			...
			$list = $this->kordinator_model->list_po($params); 
	...
	public function detail(){
			...
			$result = $this->kordinator_model->get_detail($data['id']);
