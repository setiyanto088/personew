...
class Survey_model extends CI_Model {
	...
	public function get_data_88($cat,$kuis){
	...
	public function get_data_f($cat,$kuis){
		
		$sql 	= 'SELECT a.`field_raw`,a.`type_input`,a.`code_question`,a.`sub_question`,b.* FROM `t_question_value` a
					JOIN `t_form_value` b ON a.`id_question` = b.`id_question`
					WHERE id_category = 7
					AND sub_question = '.$cat.'
					AND id_kuisioner = '.$kuis.'';
	...
	public function get_history_filter($data){
		...
		$where_hari = '';
		foreach($data['array_hari'] as $array_hari){
			$where_hari .= " day_survey LIKE '%".$array_hari."%' OR";
		}
		
		if($data['array_hari'][0] <> ''){
			$where_haris = " AND ( ".substr($where_hari,0,-2)." ) ";
		...
		if($data['date'] <> ''){
			$where_date = "AND DATE_SURVEY = '".$data['date']."'";
	...
	public function get_field($data){
		...
		$sql 	= "
		SELECT *,concat(LEFT(code_question,char_length(code_question)-3),'_fam') as fieldss,
		LEFT(code_question,char_length(code_question)-3) as raw_field FROM t_question_value
	...
	public function get_field_88($data){
	
		$sql 	= "
		SELECT *,concat(LEFT(code_question,char_length(code_question)-3),'_fam') as fieldss,
		LEFT(code_question,char_length(code_question)-3) as raw_field FROM t_question_value
		WHERE `id_category` = ".$data."
		AND sub_question IN ('1','sp')
		ORDER BY seq ASC
		";
	...
	public function get_field_f($data){
		
		$sql 	= "
		SELECT * FROM t_question_value
		WHERE `id_category` = 7 AND 
		sub_question = ".$data."
		ORDER BY seq ASC
		";
	...
	public function get_field_f_s7($data){
	...
	public function get_field_f_s8($data){
		
		$sql 	= "
		SELECT *,concat(LEFT(code_question,char_length(code_question)-3),'_fam') as fieldss,
		LEFT(code_question,char_length(code_question)-3) as raw_field FROM t_question_value
		WHERE `id_category` = 8 AND 
		sub_question = ".$data."
		and code_question like '%_kk'
		ORDER BY seq ASC
		";
	...
	public function get_field_val_f($data){
		
		$sql 	= "
		SELECT * FROM t_question_value
		WHERE `id_category` = 7
		AND `sub_question` = ".$data."
		and mandatory = 1 
		ORDER BY seq ASC
		";
	...
	public function delete_curr_answ_f($cat,$id_qus,$sub_cat){
		
		$sql 	= "
		
		DELETE FROM t_form_value 
					WHERE id_question IN ( SELECT id_question FROM t_question_value WHERE 
					id_category = ".$cat." AND sub_question = ".$sub_cat." ) AND id_kuisioner = ".$id_qus."
