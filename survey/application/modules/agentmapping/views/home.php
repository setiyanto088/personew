									 ...
									 <table id="table_resp_ss" class="table table-striped" style="width:97%">
									  <tbody id="body_table_resp">
										<?php foreach($user as $users){ 
											...
											echo "<tr style='cursor: pointer;' onClick='get_resume(\"".$users['id']."|".$users['nama']."\")'><td width='40%'><div class='row'><div class='col-md-2'><img style='border-radius:20%' src='".base_url().'uploads/profile/'.$users['image']."' alt='profile'/></div><div class='col-md-6' style='margin-left:10px'><h4>".$users['nama']."</4><h5>".$location[0]['location_name']."</h5></div></div></td><td style='text-align:right'><h5> > </h5></td></tr>";
										} ?>
									  </tbody>
									</table>
								</div>
						...
						<div class="col-md-12 grid-margin " style="">
							<div class="card">
								...
								<?php foreach($kecamatan as $kecamatan_data){ ?>
										<br>
											<div class="col-md-12"><?php echo $kecamatan_data['kota']; ?></div>
											<?php foreach($kecamatan_data['kecamatan'] as $array_familys){ ?>
											 <div class="col-md-6" >
											  <div class="form-check">
												  <label class="form-check-label">
													<input type="checkbox" name="kecamatan" id="<?php echo str_replace(" ","_",$array_familys['KECAMATAN_DAGRI']); ?>" value="<?php echo $array_familys['KECAMATAN_DAGRI']; ?>" class="form-check-input" >
													<?php echo $array_familys['KECAMATAN_DAGRI']; ?>
												  </label>
												</div>
											</div>
...
<script>
			function mapping_user(){
				...
				var urls = "<?php echo base_url('agentmapping/insert_mapping');?>";
				var values = '';
			...
			function get_resume(users){
				...
				$.ajax({
					type:'POST',
					url: '<?php echo base_url('agentmapping/get_resume');?>',
					data:formData,
					cache:false,
					contentType: false,
					processData: false,
						...
						if (response.success == true) {
							...
							// }).then(function () {
								...
								for (i = 0; i < arr.length; i++) {
									...
									$('#'+arr[i]['KECAMATAN']).prop('checked', true);
