...
class Agentperformance extends MX_Controller {

    public function __construct()
	{
		parent::__construct();
		$this->load->model('agentperformance_model');		
	...
	public function index()
	{
			...
			$data['user'] = $this->agentperformance_model->get_user_stat($session_val['user_id']);
			$data['location'] =$this->agentperformance_model->get_location($session_val['idlokasi']);
			$data['get_history'] = $this->agentperformance_model->get_history($user_id); 
			$data['get_chart'] = $this->agentperformance_model->get_chart($user_id); 
			...
			//$this->template->load('maintemplate', 'agentperformance/views/detail', $data);
			$this->template->load('maintemplate', 'agentperformance/views/home', $data);
	...
	public function get_resume(){
		...
		$get_resume = $this->agentperformance_model->get_resume($users); 
		$data['get_history'] = $this->agentperformance_model->get_history($users); 
		$data['get_chart'] = $this->agentperformance_model->get_chart($users); 
		...
		$result = array('success' => true, 'message' => '', 'html' => $data , 'chart' => $array_val, 'chart_label' => $array_data, 'his' => $html);
		...
		// }else {
			...
			//if (!isset($upload_error)) {
			   ...
			   // echo $password_generated; 
				...
				$last_id = $this->agentperformance_model->get_last_user();
				$last_id_group = $this->agentperformance_model->get_last_group();
				...
				$result = $this->agentperformance_model->add_user($data);
				//print_r($result);die;
				...
				$result2 = $this->agentperformance_model->add_user_group($data2);
				
				if ($result > 0) {
					
					$location =$this->agentperformance_model->get_location($idlokasi);
					$user = $this->agentperformance_model->get_user($idlokasi);
	...
	public function edit_user() {
				...
				$result = $this->agentperformance_model->edit_user($data);
				...
				if( $this->input->post('password', TRUE) == "00000000" ){
					$result2 = $this->agentperformance_model->edit_group_wop($data2);
				}else{
					$result2 = $this->agentperformance_model->edit_group_wp($data2);
				}
	...
	function lists() {
		 ...
		 // print_r($params);die;
			...
			$list = $this->agentperformance_model->list_po($params); 
	...
	public function detail(){
			...
			$result = $this->agentperformance_model->get_detail($data['id']);
