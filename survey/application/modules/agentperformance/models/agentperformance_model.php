...
class Agentperformance_model extends CI_Model {
	...
	public function get_user_stat($id){
		...
		SELECT id,nama,SUM(bersedia) AS bersedia, SUM(`interview`) AS interview,SUM(call_resp) AS total_call,IF(resp IS NULL,0,resp) AS resp   FROM (
			SELECT a.id,a.nama,IF(e.respond = 7,1,0 ) AS bersedia,IF(f.status_survey = 1,1,0) AS interview,
			IF(e.id_outbound IS NULL,0,1) AS call_resp,resp FROM u_user a
			...
			JOIN  (SELECT a.* FROM t_user_location a 
			 ...
			 LEFT JOIN `mapping_kecamatan` g ON a.id = g.`ID_USER`
			 LEFT JOIN ( SELECT KECAMATAN_DAGRI,COUNT(*) AS resp FROM `RESPONDENT_SURVEY_CLEAR` GROUP BY `KECAMATAN_DAGRI`) h ON g.`KECAMATAN` = h.`KECAMATAN_DAGRI`
