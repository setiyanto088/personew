      ...
      <div class="main-panel">
			 ...
			 <div class="col-md-12 grid-margin" class="survey_page_1">
              <div class="card">
				...
				<div class="row" >
						...
						-->
						<input type="hidden" id="list_location_text" name="list_location_text" value="<?php echo $list_location; ?>" />
						<div class="col-md-2" style="margin-top:20px" >
						<select class="form-control" name="list_location" id="list_location"  style="width:100%" data-placeholder="Kota"  > 
								<option value="" selected >Semua Kota</option>
												<?php  foreach($location as $locations){
													...
													if($locations['location_name'] == $filter['list_location']){
														echo "<option value='".$locations['location_name']."' Selected >".$locations['location_name']." </option>";
													}else{
						...
						<div class="col-md-2" style="margin-top:20px" >
							<select class="form-control" id="respond" placeholder="respond" style="" >
											<option value="" <?php if($filter['respond']==""){ echo "selected"; } ?> >Semua Status</option>
											<option value="1" <?php if($filter['respond']=="1"){ echo "selected"; } ?>>Valid</option>
											<option value="3" <?php if($filter['respond']=="3"){ echo "selected"; } ?>>Valid Supervisor</option>
											<option value="2" <?php if($filter['respond']=="2"){ echo "selected"; } ?>>Tidak Valid</option>
											<option value="0" <?php if($filter['respond']=="0"){ echo "selected"; } ?>>Belum Tervalidasi</option>
										  </select>
						</div>
						...
						<div class="col-md-2" style="margin-top:20px" >
							<input type="text" class="form-control" id="start_date" placeholder="Periode Awal" style="" value="<?php echo $filter['start_date']; ?>"  />
						</div>
						
						<div class="col-md-2" style="margin-top:20px" >
							<input type="text" class="form-control" id="end_date" placeholder="Periode Akhir" style="" value="<?php echo $filter['end_date']; ?>"  />
						</div>
						...
						<div class="col-md-2" style="margin-top:20px" >
							<div class="btn-group" role="group" aria-label="Basic example" style="margin:auto" >
							  ...
							  <button type="button" class="  btn btn-info" onClick="reset_filter()">Reset</button>
							</div>
						</div>
							...
							<?php foreach($get_history_sum as $table_datas){ 	
												 ...
												 ?>
													 ...
													 <tr>
														...
														<td><?php echo $table_datas['valid_supervisor_sum']; ?></td>
														<td><?php echo $table_datas['valid_sum']; ?></td>
														<td><?php echo $table_datas['valid_sum']+$table_datas['invalid_sum']+$table_datas['valid_supervisor_sum']+$table_datas['unvalidate_sum']; ?></td>
		...
		<script>
			...
			function save_res(outbound){
				...
				$.ajax({
						type: "POST",
						url: "<?php echo base_url(); ?>qc_surveyor/edit_schedule",
			...
			function reset_filter(){
				...
				var formData = new FormData();
						var urls = "<?php echo base_url('qc_surveyor/filter_jadwal'); ?>";
						...
						$.ajax({
									...
									success: function(response) {
										...
										console.log(obj.html);
										 //window.location.href = "<?php echo base_url() . 'qc_surveyor/new_survey'; ?>";
										 $("#tabel_front").html('');
			...
			function start_survey(id_outbound){
				...
				swal({
					...
					cancelButtonText: 'Tidak'
						...
						var formData = new FormData();
						var urls = "<?php echo base_url('qc_surveyor/insert_header_survey'); ?>";
						...
						$.ajax({
									...
									processData: false,
									success: function(response) {
										 window.location.href = "<?php echo base_url() . 'qc_surveyor/new_survey'; ?>/"+id_outbound;
									}
						});
			...
			function check_outbound(id_outbound){
				...
				//window.location.href = "<?php echo base_url() . 'qc_surveyor/check_survey'; ?>/"+id_outbound;
				...
				var url = '<?php echo base_url(); ?>qc_surveyor/check_survey'; 
				 ...
				 $("#laod").append(' <img id="loading" src="<?php echo base_url();?>assets/urate-frontend-master/assets/images/icon_loader.gif">');
				  var form = $("<form action='" + url + "' method='post'>" +
					...
					"<input type='hidden' name='list_location' value='" + list_location + "' />" +
				...
				// swal({
				  ...
				  // }).then(function() {
						...
						// var formData = new FormData();
						// var urls = "<?php echo base_url('qc_surveyor/delete_outbound'); ?>";
			...
			function get_agent(){
						...
						var formData = new FormData();
						var urls = "<?php echo base_url('qc_surveyor/get_agent'); ?>";
