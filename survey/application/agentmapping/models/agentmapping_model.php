...
class Agentmapping_model extends CI_Model {
	...
	public function get_location_user($user){

		$sql = "
		select location_name from t_location a
		join u_user b on a.id_location = b.lokasi
		where id = ".$user."
	";
	...
	public function get_kecamatan_user($location,$user){
		
		$sql 	= '
		select REPLACE(KECAMATAN," ","_") AS KECAMATAN,ID_USER from (
			SELECT A.KECAMATAN,b.ID_USER FROM (
			SELECT DISTINCT(KECAMATAN_DAGRI) KECAMATAN FROM RESPONDENT_SURVEY_CLEAR
			WHERE KOTA_KABUPATEN_DAGRI = "'.$location.'"
			) A LEFT JOIN mapping_kecamatan b on A.KECAMATAN = b.KECAMATAN
			AND b.ID_USER = '.$user.'
			AND b.ID_USER IS NOT NULL
			ORDER BY A.KECAMATAN
		) a where ID_USER IS NOT NULL
	...
	public function mapping_kecamatan($data)
	{
		...
		$this->db->insert_batch('mapping_kecamatan', $data);
	...
	public function delete_mapping_kecamatan($id_user)
	{
		// $sql 	= 'insert into t_eksternal  (kode_eksternal,eksternal_address,name_eksternal,phone_1,fax,bank1,rek1,bank2,rek2,bank3,rek3,type_eksternal) values (?,?,?,?,?,?,?,?,?,?,?,?)';
		...
		$this->db->delete('mapping_kecamatan');
